...
// URL do backend â€” troca aqui quando migrar de servidor
const BACKEND = "https://vortex-backend1.onrender.com";


// ==============================
// Session ID Ãºnico por dispositivo â€” identifica o usuÃ¡rio sem login
...
// Headers padrÃ£o com Session-ID para todos os requests
...
async function callClaude(systemPrompt, userPrompt, historico=[]) {
  ...
  try {
    ...
    throw new Error("?? Backend offline â€” inicie o servidor Python na porta 8082");
  }
...
const NICHOS_OB = ["Terror / True Crime","Gaming","Educacional","Humor","Lifestyle","Tecnologia","Fitness","CulinÃ¡ria","FinanÃ§as","Beleza","MÃºsica","Esportes","Empreendedorismo","Viagem","Outro"];
const PLATS_OB = ["TikTok","Instagram","YouTube","YouTube Shorts","Twitter / X","Kwai","Pinterest"];
const DIAS_OB = ["Seg","Ter","Qua","Qui","Sex","SÃ¡b","Dom"];
const TONS_OB = ["DescontraÃ­do","Educativo","EngraÃ§ado","Motivacional","Direto","Storytelling","PolÃªmico","Ã�ntimo"];
...
const GRUPOS = [
  { label: "Criar", itens: [
    ...
    { id:"tendencias",    label:"TendÃªncias",         icon:"??" },
    { id:"imagens",       label:"Imagens IA",         icon:"??" },
    { id:"videos",        label:"VÃ­deos IA",          icon:"??" },
  ]},
  { label: "Ferramentas", itens: [
    { id:"video_faceless",label:"VÃ­deo Faceless",     icon:"??" },
    { id:"voz",           label:"Clonar Voz",         icon:"???" },
    { id:"musica",        label:"Gerar MÃºsica",       icon:"??" },
  ]},
  { label: "Analisar", itens: [
    { id:"analise",       label:"AnÃ¡lise de Perfil",  icon:"??" },
...
const CONTA = [
  ...
  { id:"creditos",  label:"CrÃ©ditos",       icon:"??" },
  { id:"config",    label:"ConfiguraÃ§Ãµes",  icon:"??" },
];
const BOTTOM_NAV = [{ id:"chat",label:"Chat",icon:"?" },{ id:"roteiro",label:"Roteiro",icon:"??" },{ id:"tendencias",label:"Trends",icon:"??" },{ id:"score_viral",label:"Score",icon:"?" },{ id:"analise",label:"AnÃ¡lise",icon:"??" }];
const PAGE_META = {
  chat:{title:"Vortex Chat",sub:"IA conversacional"},dashboard:{title:"Dashboard",sub:"Painel principal"},
  analise:{title:"AnÃ¡lise de Perfil",sub:"Redes sociais"},
  analisar_video:{title:"AnÃ¡lise de VÃ­deo",sub:"Score e diagnÃ³stico"},
  video_faceless:{title:"VÃ­deo Faceless",sub:"1 clique â€” automÃ¡tico"},roteiro:{title:"Gerar Roteiro",sub:"Roteiros & Hooks"},
  tendencias:{title:"TendÃªncias Virais",sub:"2026"},imagens:{title:"Gerador de Imagens",sub:"IA Generativa"},
  videos:{title:"Gerador de VÃ­deos",sub:"WaveSpeed AI"},voz:{title:"Clonagem de Voz",sub:"ElevenLabs"},
  
  memoria:{title:"MemÃ³ria do Vortex",sub:"PreferÃªncias"},personagens:{title:"Meus Personagens",sub:"Biblioteca"},
  projetos:{title:"Meus Projetos",sub:"HistÃ³rico"},creditos:{title:"Comprar CrÃ©ditos",sub:"Planos"},
  config:{title:"ConfiguraÃ§Ãµes",sub:"Sistema"},
};

// ==============================
const QC_NICHOS = ["Terror","Gaming","Humor","Lifestyle","Educacional","Fitness","Tecnologia","CulinÃ¡ria"];
const QC_FORMATOS = [
  ...
  { id:"serie",label:"SÃ©rie de 3 vÃ­deos",emoji:"??" },
];
function QuickCreate({ onSend }) {
  ...
  const gerar=useCallback(async()=>{
    ...
    try {
      const map={
        hooks:`Crie 5 hooks virais para ${nicho}${tema?` sobre "${tema}"`:""}.Para cada: texto (mÃ¡x 7 palavras), emoÃ§Ã£o ativada, por que funciona.`,
        ...
        legenda:`Crie legenda viral para ${nicho}${tema?` sobre "${tema}"`:""}.1Âª linha impactante, corpo, CTA, 15 hashtags.`,
        serie:`Crie sÃ©rie 3 vÃ­deos para ${nicho}${tema?` sobre "${tema}"`:""}.Cada um: tÃ­tulo, hook, estrutura.`,
      };
      setResult(await callClaude("VocÃª Ã© o Vortex, especialista em conteÃºdo viral. Responda em portuguÃªs com markdown.",map[fmt]));
      setStep(4);
  ...
  return (
    <>
      <button className="chat-action-btn" onClick={()=>setOpen(true)} title="CriaÃ§Ã£o RÃ¡pida">
      ...
      {open&&(
        <div style={mo.overlay} onClick={e=>e.target===e.currentTarget&&close()}>
          <div style={mo.modal}>
            <div style={mo.hd}>
              <span style={mo.hdT}>? CriaÃ§Ã£o RÃ¡pida</span>
              <div style={mo.prog}>{[0,1,2].map(i=><div key={i} style={mo.dot(step===i?"cur":step>i?"done":"")}/>)}</div>
              <button style={mo.cls} onClick={close}>?</button>
            </div>
            {step===0&&<div style={mo.body}><span style={mo.lbl}>Qual Ã© o seu nicho?</span><div style={mo.chips}>{QC_NICHOS.map(n=><button key={n} style={mo.chip(nicho===n)} onClick={()=>setNicho(n)}>{n}</button>)}</div><button style={mo.btn} onClick={()=>setStep(1)} disabled={!nicho}>Continuar ?</button></div>}
            ...
            {step===3&&<div style={{...mo.body,...mo.loading}}><div style={mo.spin}><div className="spinner" style={{width:40,height:40,border:"2px solid rgba(124,92,252,.2)",borderTopColor:"#7c5cfc",borderRadius:"50%",animation:"spin .65s linear infinite"}}/><span style={{fontSize:20,position:"absolute"}}>?</span></div><div style={{fontFamily:"'Syne',sans-serif",fontSize:14,fontWeight:700,color:"#f1eeff"}}>Gerando com IA...</div><div style={{fontSize:11,color:"#524d72"}}>{fmtObj?.label} Â· {nicho}</div></div>}
...
function sessionTitle(msgs) { const u=msgs.find(m=>m.role==="user"); if(!u) return "Nova conversa"; return u.text.slice(0,44)+(u.text.length>44?"â€¦":""); }
...
function HistoryPanel({ sessions, activeId, onLoad, onNew, onDelete }) {
  return (
    <aside style={HS.panel}>
      <div style={HS.hd}>
        <span style={HS.lbl}>HistÃ³rico</span>
        ...
        {sessions.map(s=>(
          <div key={s.id} style={HS.item(s.id===activeId)} onClick={()=>onLoad(s.id)}>
            <div style={HS.inf}>
              ...
              <span style={HS.meta}>{s.msgs} msgs Â· {s.date}</span>
            </div>
...
// ==============================
const INITIAL_MSGS = [{ role:"assistant", text:"OlÃ¡! Sou o **Vortex**, sua IA para criaÃ§Ã£o de conteÃºdo viral. ??\n\nPosso te ajudar a criar roteiros, analisar perfis, encontrar tendÃªncias e muito mais.\n\nComo posso te ajudar hoje?" }];
const SUGESTOES = ["Crie um roteiro viral para TikTok sobre terror","Quais sÃ£o as tendÃªncias de 2026?","Me dÃª 5 hooks poderosos para Reels","Analise meu nicho de gaming"];
...
function detectIntent(text) {
  ...
  // Detectar quando usuÃ¡rio quer texto, nÃ£o geraÃ§Ã£o
  const pedindoTexto = /prompt|me d[aÃ¡] o|escreve o|qual seria|me explica|como funciona|o que e|exemplo de|do roteiro|desse roteiro/.test(t);

  // -- IMAGEM â€” sÃ³ geraÃ§Ã£o explÃ­cita ------------------------------
  ...
  // -- VÃ�DEO + VOZ â€” combina os dois ------------------------------
  const temVideo = /(vÃ­deo|video|clipe|clip|reels?|short|animaÃ§Ã£o)/.test(t);
  ...
  // -- VÃ�DEO simples -----------------------------------------------
  const geraVideo = /^(gera|cria|faz|gerar|criar)\s+(um?\s+)?(vÃ­deo|video|clipe|reel|short)/i.test(t) ||
                    /(gera|cria)\s+.*?(vÃ­deo|video)/i.test(t);
  if(geraVideo && !pedindoTexto) return "video";

  // -- VOZ/Ã�UDIO â€” narraÃ§Ã£o explÃ­cita -----------------------------
  const geraVoz = /^(narra|narrar|lÃª|leia|fala|dubla)\s+/i.test(t) ||
                  /(narr(a|ar|aÃ§Ã£o)|dubla(gem|r)|leitura em voz|gera (o )?Ã¡udio|tts|text.to.speech)/.test(t);
...
function extractTextToNarrate(userRequest, history) {
  // Se o usuÃ¡rio pediu pra narrar algo da conversa anterior
  const ref = /(narra|lÃª|leia|fala|dubla|voice).*(isso|esse|esse roteiro|esse texto|o roteiro|acima|anterior)/i;
  if (ref.test(userRequest)) {
    // Pega o Ãºltimo texto do assistente
    const last = [...history].reverse().find(m => m.role === "assistant");
    return last ? last.text.replace(/[*#_`]/g,"").trim() : userRequest;
  }
  // Remove o comando e deixa sÃ³ o texto
  return userRequest
    .replace(/^(narra|leia|fala|dubla|voice|narrar|ler|falar)\s*(isso|esse texto|o texto|em voz alta)?\s*/i,"")
    .trim() || userRequest;
}

// Detecta configuraÃ§Ãµes de voz do pedido
function extractVoiceConfig(userRequest) {
  const t = userRequest.toLowerCase();
  const config = { voice_id: "pNInz6obpgDQGcFmaJgB", stability: 0.5, similarity: 0.75 }; // padrÃ£o Adam
  ...
  if (/(animado|energÃ©tico|hype|animada)/.test(t)) { config.stability = 0.3; config.similarity = 0.8; }
...
function buildChatSystem(profile, memory, history, modo) {
  ...
  const idioma   = profile.idioma || "PortuguÃªs";

  // MemÃ³ria do usuÃ¡rio â€” estilo, evitar, CTA, referÃªncias
  ...
  // HistÃ³rico recente para contexto
  ...
  // -- MODO ASSISTENTE â€” IA geral, completa, sem restriÃ§Ã£o de tema --
  if(modo === "assistente") {
    const lines = [
      "VocÃª Ã© o VORTEX AI â€” inteligÃªncia artificial completa, equivalente ao Claude, Gemini e ChatGPT.",
      "",
      "CAPACIDADES TOTAIS:",
      "Responda qualquer pergunta de qualquer Ã¡rea: tecnologia, ciÃªncia, histÃ³ria, filosofia, negÃ³cios,",
      "medicina, direito, matemÃ¡tica, programaÃ§Ã£o, criatividade, produtividade, idiomas e muito mais.",
      "",
      "PERSONALIDADE:",
      "- Direto e inteligente â€” sem enrolaÃ§Ã£o, sem robÃ³tica",
      "- Honesto â€” se nÃ£o souber, diz claramente em vez de inventar",
      "- Tom adaptÃ¡vel â€” tÃ©cnico quando necessÃ¡rio, descontraÃ­do quando o usuÃ¡rio quer",
      "- Contextual â€” mantÃ©m coerÃªncia com o que foi dito na conversa",
      "",
      "REGRAS CRÃ�TICAS:",
      "- NUNCA invente dados, estatÃ­sticas ou fatos",
      "- NUNCA diga que Ã© Claude, Gemini, GPT â€” vocÃª Ã© o VORTEX",
      "- Quando perguntarem sobre VOCÃŠ (suas capacidades) ? responda sobre VOCÃŠ",
      "- Responda SEMPRE em " + idioma,
    ];
    if(nome) lines.push("- UsuÃ¡rio: " + nome);
    if(nicho) lines.push("- Contexto do usuÃ¡rio: criador de conteÃºdo de " + nicho);
    if(recentMsgs) lines.push("- Contexto da conversa: " + recentMsgs);
    return lines.filter(Boolean).join("\n");

  }

  // -- MODO CRIADOR â€” especialista em conteÃºdo viral --------------
  const lines = [
    "VocÃª Ã© o VORTEX AI â€” o assistente de criaÃ§Ã£o de conteÃºdo viral mais inteligente do Brasil.",
    "VocÃª entende algoritmos, tendÃªncias, psicologia do scroll e o mercado brasileiro profundamente.",
    "",
  ];
  ...
  if(nome || nicho || plats) {
    ...
    if(publicoAlvo) lines.push("- PÃºblico-alvo: " + publicoAlvo);
    if(link)   lines.push("- Perfil: " + link);
    lines.push("");
  }

  // MemÃ³ria personalizada
  const memoriaItems = [
    ...
    referencias     && "- ReferÃªncias/inspiraÃ§Ãµes: " + referencias,
  ].filter(Boolean);

  if(memoriaItems.length > 0) {
    lines.push("MEMÃ“RIA DO CRIADOR (use sempre):");
  ...
  lines.push(
    ...
    "- TendÃªncias em tempo real por nicho e plataforma",
    "- EstratÃ©gias de crescimento por algoritmo (TikTok, Reels, Shorts)",
    "- Copywriting que converte â€” gatilhos emocionais precisos",
    "- AnÃ¡lise de mÃ©tricas e diagnÃ³stico de perfis",
    "",
    "REGRAS:",
    "- Personalize TUDO ao perfil do criador acima",
    "- Responda em " + idioma + " natural â€” sem robÃ³tica",
    "- NUNCA comeÃ§a com 'Certamente!', 'Claro!', 'Ã“tima pergunta!'",
...
async function vortexRouter(userText, profile, memory, history, historicoBackend=[]) {
  ...
  // Chat â€” monta histÃ³rico e system prompt sem template literals complexos
  const personagemCtx = personagem
    ? "\n\nPERSONAGEM ATIVO: " + personagem.nome + " â€” " + (personagem.personalidade||"") + " â€” voz: " + personagem.voice_nome
    : "";
  const modoAtual = window.__vortexModo || "criador";
  const system = buildChatSystem(profile, memory, history, modoAtual) + personagemCtx;

  // Envia sÃ³ a mensagem atual â€” histÃ³rico vai separado
...
async function generateImage(prompt, modelo="pollinations", modeloObj=null) {
  ...
  // Se for modelo AIML â€” usa endpoint dedicado
  ...
  // Modelo padrÃ£o â€” endpoint free
  try {
    res = await fetch(`${BASE}/gerar-imagem-free`, {
      method:"POST", headers:{"Content-Type":"application/json"},
      body: JSON.stringify({prompt, modelo}),
    });
  } catch { throw new Error("?? Backend offline â€” inicie o servidor Python"); }
  if(!res.ok){const e=await res.json().catch(()=>({}));throw new Error(e?.detail||`HTTP ${res.status}`);}
  const d = await res.json();
...
// ==============================
const VOZES_PRONTAS = [
  { id:"pNInz6obpgDQGcFmaJgB", nome:"Adam",    desc:"Masculina grave, narraÃ§Ã£o",  emoji:"???" },
  { id:"EXAVITQu4vr4xnSDxMaL", nome:"Sarah",   desc:"Feminina suave, conversa",   emoji:"??" },
  { id:"TX3LPaxmHKxFdv7VOQHJ", nome:"Liam",    desc:"Masculina jovem, energÃ©tico", emoji:"?" },
...
// Monta o prompt de imagem/vÃ­deo com consistÃªncia de personagem
...
// Detecta mudanÃ§as pedidas pelo usuÃ¡rio em novo vÃ­deo
function extractMudancas(userText) {
  const t = userText.toLowerCase();
  const mudancas = {};
  if (t.includes("cenario") || t.includes("cenÃ¡rio")) {
  ...
  const exprs = ["sorrindo","sÃ©rio","triste","animado","com raiva","surpreso","feliz"];
  ...
  if (t.includes("estÃºdio"))  mudancas.cenario = "studio, clean background";
  if (t.includes("noite"))    mudancas.cenario = (mudancas.cenario||"") + " night time";
  return mudancas;
}

// Modal de criaÃ§Ã£o/ediÃ§Ã£o de personagem
    ...
    <div style={{position:"fixed",inset:0,zIndex:500,background:"rgba(4,3,14,.85)",backdropFilter:"blur(8px)",display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div style={{width:"100%",maxWidth:520,background:"var(--bg3)",border:".5px solid rgba(123,47,255,.3)",borderRadius:"var(--r-xl)",maxHeight:"90vh",overflow:"hidden",display:"flex",flexDirection:"column"}}>
        ...
        <div style={{display:"flex",gap:4,padding:"12px 16px 0",flexShrink:0}}>
          {[["aparencia","?? AparÃªncia"],["voz","??? Voz"],["padrao","?? PadrÃµes"]].map(([t,l])=>(
        ...
        <div style={{flex:1,overflowY:"auto",padding:"1.25rem 1.5rem"}}>
          {tab==="aparencia" && (
            <>
              ...
              <div className="field">
                <label className="label">DescriÃ§Ã£o visual (aparÃªncia fÃ­sica)</label>
                <textarea className="input" rows={4}
                  placeholder="Ex: homem jovem, 25 anos, cabelo preto curto, olhos castanhos, pele morena, expressÃ£o sÃ©ria, estilo urbano..."
                  value={dados.aparencia} onChange={e=>set("aparencia",e.target.value)}/>
                <div style={{fontSize:10,color:"var(--text3)",marginTop:4}}>Quanto mais detalhado, mais consistente o personagem vai ficar entre os vÃ­deos</div>
              </div>
              <div className="field">
                <label className="label">Personalidade / Jeito de falar</label>
                <textarea className="input" rows={2}
                  placeholder="Ex: engraÃ§ado, direto, gÃ­rias cariocas, sempre animado..."
          ...
          {tab==="voz" && (
            <>
              ...
              <div style={{background:"rgba(123,47,255,.06)",border:".5px solid rgba(123,47,255,.2)",borderRadius:10,padding:"12px 14px"}}>
                ...
                <div style={{fontSize:11,color:"var(--text3)",marginBottom:8}}>Grave 30 segundos da sua voz ou do personagem. O Vortex clona e usa sempre que gerar conteÃºdo.</div>
                <button className="btn btn-sm btn-ghost" onClick={()=>alert("Integre com ElevenLabs Voice Cloning API")}>
                  ??? Fazer upload de Ã¡udio para clonar
                </button>
              </div>
              <div className="field" style={{marginTop:12}}>
                <label className="label">Tom de fala / instruÃ§Ãµes</label>
          ...
          {tab==="padrao" && (
            <>
              <div className="field">
                <label className="label">CenÃ¡rio padrÃ£o</label>
                <input className="input" placeholder="Ex: estÃºdio moderno, fundo preto com neon roxo..."
                  value={dados.cenario_padrao} onChange={e=>set("cenario_padrao",e.target.value)}/>
              </div>
              <div className="field">
                <label className="label">Roupa padrÃ£o</label>
                <input className="input" placeholder="Ex: camiseta preta, bonÃ©, corrente dourada..."
                  value={dados.roupa_padrao} onChange={e=>set("roupa_padrao",e.target.value)}/>
              </div>
              <div className="field">
                <label className="label">ExpressÃ£o padrÃ£o</label>
                <div className="chips">
                  {["neutro","sorrindo","sÃ©rio","animado","confiante"].map(ex=>(
              ...
              <div className="field">
                <label className="label">Pose padrÃ£o</label>
                <input className="input" placeholder="Ex: de frente, olhando para cÃ¢mera, braÃ§os cruzados..."
        ...
        <div style={{padding:"1rem 1.5rem",borderTop:".5px solid var(--border)",display:"flex",gap:8,flexShrink:0}}>
          ...
          <button className="btn" style={{flex:2}} onClick={()=>onSave({...dados,id:dados.id||Date.now().toString(36)})}>
            ?? {personagem?"Salvar alteraÃ§Ãµes":"Criar personagem"}
          </button>
        </div>
      </div>
    </div>
...
// Componente da pÃ¡gina de personagens
  ...
  return (
    <div className="page">
      <div className="page-header">
        ...
        <p className="page-sub">Crie personagens consistentes para seus vÃ­deos e conteÃºdos.</p>
      </div>
      ...
      {ativo && (
        <div style={{background:"rgba(123,47,255,.08)",border:".5px solid rgba(123,47,255,.3)",borderRadius:"var(--r-lg)",padding:"12px 16px",marginBottom:"1.25rem",display:"flex",alignItems:"center",gap:12}}>
          ...
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:11,color:"var(--text3)"}}>Voz: {ativo.voice_nome} Â· Todos os vÃ­deos usarÃ£o este personagem</div>
          </div>
        ...
        <div key={p.id} className="card" style={{display:"flex",alignItems:"flex-start",gap:14,background:ativo?.id===p.id?"rgba(123,47,255,.06)":"var(--bg3)",border:ativo?.id===p.id?"1px solid rgba(123,47,255,.3)":".5px solid var(--border)"}}>
          ...
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:12,color:"var(--text3)",marginBottom:4}}>
              ...
              {p.cenario_padrao&&` Â· ?? ${p.cenario_padrao.slice(0,30)}`}
            </div>
...
const ESTILOS_EDICAO = [
  { id:"viral_tiktok",   nome:"?? Viral TikTok",    desc:"Legendas palavra a palavra, cortes rÃ¡pidos, zoom",     cor:"#f472b6" },
  { id:"cinematografico",nome:"?? CinematogrÃ¡fico",  desc:"Cortes suaves, texto elegante, color grade escuro",    cor:"#a78bfa" },
  { id:"educacional",    nome:"?? Educacional",      desc:"Legendas limpas, destaques em palavras-chave",         cor:"#22d3ee" },
  { id:"terror",         nome:"?? Terror/Suspense",  desc:"Glitch, ruÃ­do, texto tremendo, cortes abruptos",       cor:"#ef4444" },
...
// TranscriÃ§Ã£o via AssemblyAI (ou fallback mock para dev)
async function transcribeVideo(videoUrl) {
  // Mock words â€” em produÃ§Ã£o o backend chama AssemblyAI
  try {
  ...
  // Fallback mock se backend nÃ£o tiver endpoint de transcriÃ§Ã£o ainda
  return [
    { text:"OlÃ¡ pessoal,",     start:0,    end:800  },
    { text:"hoje vou mostrar", start:900,  end:1800 },
    { text:"algo incrÃ­vel",    start:1900, end:2700 },
    { text:"que vai mudar",    start:2800, end:3600 },
    { text:"seu conteÃºdo!",    start:3700, end:4500 },
  ];
}
...
async function renderWithShotstack(timeline) {
  ...
  try {
    ...
    throw new Error("Backend offline â€” inicie o servidor Python");
  }
  if (!res.ok) { const e = await res.json().catch(()=>({})); throw new Error(e?.detail || `HTTP ${res.status}`); }
  const d = await res.json();
  return d.video_url;
...
// Componente de ediÃ§Ã£o de vÃ­deo
function VideoEditor({ onClose, onResult }) {
  ...
  const processar = async () => {
    setStep(2); setErro("");
    try {
      setProgMsg("?? Enviando vÃ­deo para o servidor...");
      ...
      if (!uploadRes.ok) throw new Error("Falha no upload do vÃ­deo");
      const uploadData = await uploadRes.json();
      const videoUrl = uploadData.url; // URL pÃºblica retornada pelo backend

      setProgMsg("?? Preparando ediÃ§Ã£o...");
      const words = []; // transcriÃ§Ã£o opcional

      setProgMsg("?? Montando timeline de ediÃ§Ã£o...");
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:500,background:"rgba(4,3,14,.85)",backdropFilter:"blur(8px)",display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div style={{width:"100%",maxWidth:520,background:"var(--bg3)",border:".5px solid rgba(123,47,255,.3)",borderRadius:"var(--r-xl)",maxHeight:"90vh",overflow:"hidden",display:"flex",flexDirection:"column"}}>
        ...
        {/* Header */}
        <div style={{padding:"1.25rem 1.5rem",borderBottom:".5px solid var(--border)",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
          <div style={{fontFamily:"var(--fh)",fontSize:16,fontWeight:800}}>?? Editar VÃ­deo</div>
          <button onClick={onClose} style={{background:"transparent",border:"none",color:"var(--text3)",fontSize:16,cursor:"pointer"}}>?</button>
        </div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"1.25rem 1.5rem"}}>
          ...
          {step===0 && (
            <>
              <div
                ...
                <div style={{fontFamily:"var(--fh)",fontSize:14,fontWeight:700,color:"var(--text)",marginBottom:6}}>Arraste o vÃ­deo aqui</div>
                <div style={{fontSize:12,color:"var(--text3)"}}>ou clique para selecionar Â· MP4, MOV, AVI</div>
              </div>
          ...
          {step===1 && (
            <>
              ...
              <div className="field">
                <label className="label">Escolha o estilo de ediÃ§Ã£o</label>
              ...
              <div style={{display:"flex",gap:8,marginTop:8}}>
                <button className="btn btn-ghost" onClick={()=>{setStep(0);setFile(null);setFileUrl("");}}>? Trocar vÃ­deo</button>
          ...
          {step===2 && (
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:220,gap:16,textAlign:"center"}}>
              ...
              <div style={{fontSize:11,color:"var(--text3)"}}>Pode levar atÃ© 3 minutos dependendo do vÃ­deo</div>
...
function NarrarBtn({ texto }) {
  ...
  const narrar = async () => {
    setLoading(true);
    try {
      // Limpa markdown para narraÃ§Ã£o
      ...
      if (!res || !res.ok) throw new Error("ElevenLabs indisponÃ­vel");
      const d = await res.json();
      const base64 = d.audio_url.split(",")[1];
      const blob = new Blob([Uint8Array.from(atob(base64), c => c.charCodeAt(0))], { type: "audio/mpeg" });
...
function VortexChat() {
  ...
  const enviar=useCallback(async(texto)=>{
    ...
    setChatFilePreview(null);
    try{
      // Carrega perfil e memÃ³ria do usuÃ¡rio
      const profile = loadUserProfile();
      const memory  = loadUserMemory();

      // Prompt Engine â€” detecta intenÃ§Ã£o e enriquece o prompt
      // Envia histÃ³rico formatado para o backend
      ...
      const routed = await vortexRouter(t, profile, memory, messages, historicoBackend);
        ...
        // Roteiro â€” chama endpoint dedicado com Claude diretamente
...
Verifique sua VITE_WAVESPEED_API_KEY no .env`}];
        ...
        // Gera vÃ­deo
        setMessages([...comUser, {role:"assistant", text:"?? Gerando vÃ­deo com IA... (pode levar atÃ© 2 min)", loading:true}]);
        try {
          const url = await generateVideo(routed.prompt);
          finalMsgs = [...comUser, {role:"assistant", text:`?? **VÃ­deo gerado!**

[?? Clique para assistir](${url})

*Prompt: ${routed.prompt.slice(0,80)}...*`, videoUrl: url}];
        } catch(vidErr) {
          finalMsgs = [...comUser, {role:"assistant", text:`?? Erro ao gerar vÃ­deo: ${vidErr.message}`}];
        }
      } else {
      ...
      if (e.message?.includes("401") || e.message?.includes("API Key")) {
        ...
        // Retry silencioso â€” usuÃ¡rio nÃ£o vÃª erro
        ...
        msg = "?? **Erro de conexÃ£o** â€” verifique sua internet. Se estiver com VPN, desative e tente novamente.";
      }
  ...
  const handleQuick=useCallback(texto=>{enviar(`Aqui estÃ¡ o conteÃºdo gerado:\n\n${texto}\n\nPode melhorar ou adaptar?`);},[enviar]);
  ...
  return (
    <div style={{display:"flex",height:"100%",width:"100%",overflow:"hidden",minHeight:0}}>
      ...
      <div style={{flex:1,minWidth:0,height:"100%",display:"flex",flexDirection:"column",overflow:"hidden",position:"relative"}}>
        ...
        {/* TOPBAR DO CHAT â€” seletor de modo + modelo atual */}
        <div style={{
        ...
        <div className="chat-messages">
          {/* Empty state â€” aparece quando nÃ£o tem mensagens */}
          {messages.length===0&&(
              ...
              <div>
                ...
                <div style={{fontSize:12,color:"var(--text3)",maxWidth:260,lineHeight:1.6}}>
                  {window.__vortexModo==="assistente"
                    ?"Sou o Vortex AI â€” respondo qualquer pergunta como Claude, GPT e Gemini."
                    :"Roteiros, estratÃ©gias, hooks, tendÃªncias â€” tudo em um lugar."}
                </div>
              </div>
              {/* SugestÃµes rÃ¡pidas no empty state */}
          ...
          {messages.map((m,i)=>(
            <div key={i} className={`msg-row ${m.role==="user"?"user":""}`}>
              ...
              <div>
                {m.role==="assistant"&&<div className="msg-name">Vortex AI {m.modelo?<span style={{fontSize:9,color:"rgba(123,47,255,.6)",marginLeft:4}}>Â· {m.modelo}</span>:""}</div>}
                    ...
                    <>
                      ...
                      {m.videoUrl && (
                        <div style={{marginTop:8}}>
                          ...
                          <a href={m.videoUrl} download="vortex-video.mp4" style={{display:"inline-block",marginTop:6,fontSize:11,color:"var(--cyan)",textDecoration:"none",background:"rgba(34,211,238,.1)",border:".5px solid rgba(34,211,238,.3)",padding:"3px 10px",borderRadius:99}}>?? Baixar vÃ­deo</a>
                        </div>
                      )}
                      {m.audioUrl && (
                        <div style={{marginTop:8,background:"rgba(123,47,255,.08)",border:".5px solid rgba(123,47,255,.25)",borderRadius:12,padding:"10px 14px"}}>
                          <div style={{fontSize:11,color:"var(--purple2)",marginBottom:6,fontWeight:600}}>??? NarraÃ§Ã£o</div>
                          <audio src={m.audioUrl} controls style={{width:"100%",height:36}}/>
                          <a href={m.audioUrl} download="vortex-audio.mp3" style={{display:"inline-block",marginTop:6,fontSize:11,color:"var(--purple2)",textDecoration:"none",background:"rgba(123,47,255,.1)",border:".5px solid rgba(123,47,255,.3)",padding:"3px 10px",borderRadius:99}}>?? Baixar Ã¡udio</a>
                        </div>
                      )}
                    </>
                  ) : (
          ...
          <div className="chat-input-outer">
            ...
            <div className="chat-input-box">
              <textarea ref={taRef} className="chat-textarea"
                placeholder={window.__vortexModo==="assistente"?"Pergunte qualquer coisa â€” como Claude, GPT e Gemini...":"Roteiro, estratÃ©gia, hook, tendÃªncia â€” o que criar hoje?"}
                value={input} rows={1}
            ...
            <div className="chat-input-actions">
              ...
              <button className="chat-action-btn" onClick={()=>setInput("Quais tendÃªncias estÃ£o bombando em ")}>
                <span style={{fontSize:16}}>??</span><span>TendÃªncia</span>
              </button>
              <button className="chat-action-btn" onClick={()=>setInput("Me dÃª ideias de conteÃºdo para ")}>
                <span style={{fontSize:16}}>??</span><span>Ideias</span>
              </button>
              <QuickCreate onSend={handleQuick}/>
            </div>
          </div>
          <div className="chat-hint">Enter para enviar Â· Shift+Enter para nova linha</div>
        </div>
      </div>
    </div>
  );
...
function VideoFaceless() {
  ...
  const NICHOS=["terror","gaming","finanÃ§as","fitness","lifestyle","tech","motivacional","curiosidades","crime real","anime"];
  ...
  return(
    <div className="page">
      <div className="page-header">
        <div className="eyebrow"><div className="eyebrow-dot"/>IA & CriaÃ§Ã£o</div>
        <h1 className="page-title">VÃ­deo Faceless <em>1 Clique</em></h1>
        <p className="page-sub">Digite o tema ? Vortex cria roteiro + imagens + narraÃ§Ã£o automÃ¡tico</p>
      </div>
      ...
      <div className="card" style={{marginBottom:"1rem",background:"linear-gradient(135deg,#0d0020,#1a003d)",border:"1px solid rgba(124,92,252,.3)"}}>
        <div style={{fontSize:13,color:"rgba(200,180,255,.8)",marginBottom:12,lineHeight:1.6}}>
          ? <strong>Como funciona:</strong> VocÃª digita o tema ? Vortex gera roteiro viral ? cria imagens para cada cena ? narra com voz profissional ? entrega tudo pronto
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6}}>
          {["?? Roteiro","?? Imagens","??? NarraÃ§Ã£o","?? Pronto"].map((s,i)=>(
      ...
      <div className="card" style={{marginBottom:"1rem"}}>
        <div className="field">
          <label className="label">Tema do vÃ­deo</label>
          <textarea className="input" rows={3}
            placeholder="Ex: Um serial killer que ninguÃ©m sabia que morava ao lado... / O segredo que a NASA esconde sobre Marte..."
        ...
        <button className="btn btn-full" onClick={gerar} disabled={loading||!tema.trim()}
          ...
          {loading?"? Criando seu vÃ­deo...":"? Gerar VÃ­deo Faceless"}
        </button>
      </div>
      ...
      {resultado&&(
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          ...
          <div className="card" style={{background:"#7C3AED18",border:"1px solid #7C3AED44"}}>
            <div style={{fontWeight:700,fontSize:14,color:"#A855F7",marginBottom:8}}>? VÃ­deo pronto!</div>
          ...
          {resultado.etapas?.audio&&(
            <div className="card">
              <div style={{fontWeight:600,fontSize:13,marginBottom:8}}>??? NarraÃ§Ã£o</div>
...
function AnalisarVideo() {
  ...
  const NICHOS=["viral","terror","gaming","finanÃ§as","fitness","lifestyle","tech","humor","educaÃ§Ã£o"];

  return(
    <div className="page">
      <div className="page-header">
        <div className="eyebrow"><div className="eyebrow-dot"/>AnÃ¡lise</div>
        <h1 className="page-title">AnÃ¡lise de <em>VÃ­deo</em></h1>
        <p className="page-sub">Cole os dados do vÃ­deo e descubra por que viralizou ou nÃ£o.</p>
      </div>

      <div className="card" style={{marginBottom:"1rem"}}>
        <div className="field">
          <label className="label">TÃ­tulo do vÃ­deo</label>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:12}}>
          ...
          <div className="field">
            <label className="label">?? ComentÃ¡rios</label>
        ...
        <button className="btn btn-full" onClick={analisar} disabled={loading||(!titulo&&!views)}
          ...
          {loading?"?? Analisando com IA...":"?? Analisar VÃ­deo"}
        </button>
      </div>
  ...
  return (
    <div className="page">
      <div className="page-header">
        ...
        <h1 className="page-title">{nome ? "OlÃ¡, "+nome.split(" ")[0]+" ??" : "Bem-vindo ao"} <em>{nome?"Vortex":"Vortex"}</em></h1>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:"1.25rem"}}>
        <div className="card" style={{textAlign:"center",padding:"1.25rem .75rem"}}>
          ...
          <div style={{fontSize:11,color:"var(--text3)",marginTop:4}}>?? CrÃ©ditos</div>
        </div>
        ...
        <div className="card" style={{textAlign:"center",padding:"1.25rem .75rem",cursor:"pointer"}} onClick={()=>setAba("videos")}>
          <div style={{fontSize:28}}>??</div>
          <div style={{fontSize:11,color:"var(--text3)",marginTop:4}}>Gerar VÃ­deo</div>
        </div>
      </div>
      ...
      <div className="card" style={{marginBottom:"1rem",background:"rgba(123,47,255,.04)",border:"0.5px solid rgba(123,47,255,.2)"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
          <div className="card-title">?? CÃ©rebro do Vortex</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:10}}>
          ...
          <div style={{textAlign:"center",padding:8,background:"rgba(255,255,255,.03)",borderRadius:8}}>
            ...
            <div style={{fontSize:10,color:"var(--text3)"}}>taxa aprovaÃ§Ã£o</div>
          </div>
          <div style={{textAlign:"center",padding:8,background:"rgba(255,255,255,.03)",borderRadius:8}}>
      ...
      <div className="card" style={{marginBottom:"1rem"}}>
        <div className="card-title" style={{marginBottom:".75rem"}}>? AÃ§Ãµes RÃ¡pidas</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          {[
            {icon:"??",label:"Criar Roteiro",aba:"roteiro"},
            {icon:"??",label:"Analisar Perfil",aba:"analise"},
            {icon:"??",label:"Ver TendÃªncias",aba:"tendencias"},
...
function AnalisePerfil({ setAba }) {
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div className="eyebrow"><div className="eyebrow-dot"/>AnÃ¡lise</div>
        <h1 className="page-title">AnÃ¡lise de <em>Perfil</em></h1>
        <p className="page-sub">Dados reais + recomendaÃ§Ãµes IA para crescer.</p>
      </div>
      ...
      <div className="card" style={{marginBottom:"1rem"}}>
        ...
        <button className="btn btn-full" onClick={analisar} disabled={loading||!perfil.trim()}>
          {loading ? "?? Analisando..." : `?? Analisar ${redeAtual?.label} â€” 2 crÃ©ditos`}
        </button>
      </div>
      ...
      {dados && (
        <>
          ...
          {(dados.analise_completa || dados.recomendacao_ia) && (
            <div className="card" style={{marginBottom:"1rem",border:".5px solid rgba(123,47,255,.2)"}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
                <div className="card-title">?? AnÃ¡lise do Vortex AI</div>
                <button onClick={()=>{
          ...
          {/* AÃ§Ãµes rÃ¡pidas */}
          <div className="card">
            <div className="card-title" style={{marginBottom:10}}>? AÃ§Ãµes rÃ¡pidas</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
...
function Roteiro() {
  const MODOS = [
    ...
    {id:"diretor",   icon:"??", label:"Diretor",   desc:"Frame a frame com direÃ§Ã£o de cÃ¢mera"},
    {id:"serie",     icon:"??", label:"SÃ©rie",     desc:"3 episÃ³dios com cliffhanger"},
    {id:"ab",        icon:"??", label:"A/B",       desc:"2 versÃµes para testar qual performa mais"},
    {id:"faceless",  icon:"??", label:"Faceless",  desc:"Sem mostrar rosto â€” narraÃ§Ã£o + imagens"},
    {id:"anuncio",   icon:"??", label:"AnÃºncio",   desc:"Roteiro de conversÃ£o para vender"},
  ];
  const FORMATOS = [
    {id:"curto",    label:"30s", desc:"1 crÃ©dito"},
    {id:"medio",    label:"60s", desc:"2 crÃ©ditos"},
    {id:"longo",    label:"3min",desc:"3 crÃ©ditos"},
    {id:"completo", label:"5min",desc:"5 crÃ©ditos"},
  ];
  const NICHOS = ["terror","gaming","finanÃ§as","fitness","lifestyle","tech","motivacional","culinÃ¡ria","humor","anime","educacional","empreendedorismo"];
  ...
  const extrairScore = (txt) => {
    const m = txt.match(/MÃ‰DIA[:\s]+(\d+(?:[.,]\d+)?)/i) ||
              txt.match(/mÃ©dia[:\s]+(\d+(?:[.,]\d+)?)/i) ||
              txt.match(/(\d+(?:[.,]\d+)?)\/10\s*$/) ;
  ...
  const gerarRoteiro = async () => {
    ...
    const etapas = [
      {p:8,  msg:"Pesquisando tendÃªncias reais..."},
      ...
      {p:95, msg:"Finalizando produÃ§Ã£o..."},
    ];
    let etapaIdx = 0;
    const tick = setInterval(()=>{
  ...
  return (
    <div className="page">
      <div className="page-header">
        ...
        <p className="page-sub">Roteiros virais com IA â€” hook, virada e score em segundos.</p>
      </div>
      ...
      <div className="card" style={{marginBottom:12}}>
        <div className="field">
          ...
          <textarea className="input" rows={3}
            placeholder={
              modo==="terror"?"Ex: O apartamento que ninguÃ©m conseguia alugar por mais de 30 dias...":
              modo==="anuncio"?"Ex: App que dobrou minha renda em 30 dias...":
              "Ex: O experimento psicolÃ³gico que provou que todos mentem..."
            }
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          ...
          <div className="field">
            <label className="label">DuraÃ§Ã£o</label>
            <div className="chips" style={{flexWrap:"wrap"}}>
              {FORMATOS.map(f=>(
        ...
        {loading&&(
          <div style={{marginBottom:12}}>
            <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:"var(--text3)",marginBottom:6}}>
              <span>
                {progresso<20?"?? Pesquisando tendÃªncias...":
        ...
        <button className="btn btn-full" onClick={gerarRoteiro} disabled={loading||!tema.trim()}>
          {loading?<><div className="spinner"/>Gerando roteiro...</>:
           `?? Gerar Roteiro ${modo==="ab"?"A/B":modo==="serie"?"em SÃ©rie":modo==="diretor"?"Diretor":""} â€” ${FORMATOS.find(f=>f.id===formato)?.desc||"2 crÃ©ditos"}`}
        </button>
      </div>
      ...
      {resultado&&(
        <div>
          ...
          {scoreViral&&(
            <div className="card" style={{marginBottom:12,background:"rgba(123,47,255,.04)",border:`.5px solid ${scoreColor}44`,textAlign:"center"}}>
              ...
              <div style={{fontSize:13,fontWeight:600,color:scoreColor,marginTop:4}}>
                {scoreViral>=9?"?? Potencial viral altÃ­ssimo":scoreViral>=7?"? Bom potencial viral":"?? Pode melhorar"}
              </div>
          ...
          {/* AÃ§Ãµes */}
          <div className="card" style={{marginBottom:12}}>
            <div className="card-title" style={{marginBottom:10}}>?? PrÃ³ximos passos</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              <button className="btn" style={{fontSize:12}} onClick={()=>{setTema(tema+" â€” versÃ£o 2");gerarRoteiro();}}>
                ?? Gerar variaÃ§Ã£o
              </button>
              ...
              <button className="btn" style={{fontSize:12,gridColumn:"1/-1"}} onClick={async()=>{
                try{
                  ...
                  const a=document.createElement("a");a.href=audioUrl;a.download="narraÃ§Ã£o-vortex.mp3";a.click();
                  alert("? NarraÃ§Ã£o gerada e baixando!");
          ...
          {feedbackDado&&<div style={{textAlign:"center",color:"#34d399",fontSize:13,padding:8}}>? Vortex aprendeu! PrÃ³ximos roteiros serÃ£o melhores.</div>}
        </div>
      )}
      ...
      {historico.length>0&&!resultado&&(
        <div className="card">
          <div className="card-title" style={{marginBottom:10}}>?? HistÃ³rico desta sessÃ£o</div>
          {historico.map((h,i)=>(
            <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",
              ...
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:10,color:"var(--text3)"}}>{h.modo} Â· {h.formato}</div>
              </div>
...
function Tendencias() {
  ...
  // Google Trends â€” carrega automÃ¡tico
  useEffect(()=>{
    ...
    try{
      if(aba==="nicho"){
        const r=await callClaude(
          "Especialista em tendÃªncias virais de redes sociais. Responda em portuguÃªs.",
          `Liste as top 8 tendÃªncias de conteÃºdo para ${nicho} no ${plataforma} em 2026. Para cada: nome, por que viraliza, dica prÃ¡tica de uso. Responda SOMENTE JSON: {"tendencias":[{"nome":"...","motivo":"...","dica":"...","emoji":"..."}]}`
        );
  ...
  const NICHOS=["terror","gaming","finanÃ§as","fitness","lifestyle","tech","motivacional","culinÃ¡ria","humor","anime"];
  ...
  return(
    <div className="page">
      <div className="page-header">
        ...
        <h1 className="page-title">TendÃªncias <em>Virais</em></h1>
        <p className="page-sub">Google Trends em tempo real + anÃ¡lise por nicho.</p>
        <div style={{display:"inline-flex",alignItems:"center",gap:6,marginTop:6,
          background:"rgba(52,211,153,.1)",border:".5px solid rgba(52,211,153,.3)",
          borderRadius:20,padding:"4px 12px"}}>
          <span style={{fontSize:12}}>?</span>
          <span style={{fontSize:11,color:"#34d399",fontWeight:600}}>GrÃ¡tis e ilimitado â€” disponÃ­vel em todos os planos</span>
        </div>
      </div>
      ...
      <div style={{display:"flex",gap:6,marginBottom:16,background:"rgba(255,255,255,.04)",borderRadius:12,padding:4}}>
        {[
          ...
          {id:"global",icon:"??",label:"TendÃªncia Global"},
        ].map(t=>(
      ...
      {aba==="nicho"&&(
        <div>
          <div className="card" style={{marginBottom:12}}>
            ...
            {erro&&<div className="erro">? {erro}</div>}
            <button className="btn btn-full" onClick={buscar} disabled={loading}>
              {loading?<><div className="spinner"/>Analisando...</>:"?? Ver top 8 tendÃªncias"}
            </button>
          </div>
          ...
          {dados?.tipo==="nicho"&&dados.lista&&(
            <div className="card">
              <div className="card-title" style={{marginBottom:12}}>
                Top 8 â€” {dados.nicho} no {dados.plataforma}
              </div>
              {dados.lista.map((t,i)=>(
      ...
      {aba==="global"&&(
        <div>
          <div className="card" style={{marginBottom:12}}>
            ...
            {erro&&<div className="erro">? {erro}</div>}
            <button className="btn btn-full" onClick={buscar} disabled={loading}>
              {loading?<><div className="spinner"/>Buscando...</>:"?? Ver tendÃªncia global"}
            </button>
          </div>
          {dados?.lista&&(
            <div className="card">
              <div className="card-title" style={{marginBottom:12}}>?? TendÃªncias Globais</div>
              {dados.lista.map((t,i)=>{
                const nome=typeof t==="object"?t.titulo||t.nome||String(t):String(t);
...
const PLANO_FREE = {
  ...
  features:[
    "50 crÃ©ditos grÃ¡tis",
    "?? TendÃªncias em tempo real â€” ILIMITADO",
    "? Score Viral â€” ILIMITADO",
    "Chat com IA (10x/dia)",
    "3 roteiros por dia",
    "5 imagens por mÃªs",
  ],
...
const PLANOS_BASE = [
  { id:"starter", nome:"Starter", emoji:"?", desc:"Para comeÃ§ar",
    ...
    features:["250 crÃ©ditos/mÃªs","25 imagens IA","4 vÃ­deos","Roteiros ilimitados","Chat com Gemini"],
    modelo:"Gemini Flash"
  },
  { id:"creator", nome:"Creator", emoji:"??", desc:"Postar todo dia",
    creditos:900, imgs:90, vids:12, voz:true,
    promo:27, normal:49, cor:"#22d3ee", anim:"galaxia", animLabel:"?? GalÃ¡xia", popular:true,
    preco_mensal:27, preco_anual:22, creditos_anual:1100,
    features:["900 crÃ©ditos/mÃªs","90 imagens IA","12 vÃ­deos","Clonagem de voz","Chat com GPT-4o","Score Viral","TendÃªncias em tempo real"],
  ...
  { id:"pro", nome:"Pro", emoji:"???", desc:"Alto volume",
    ...
    features:["2.500 crÃ©ditos/mÃªs","250 imagens IA","35 vÃ­deos","Clonagem de voz","Chat com Claude Haiku","AnÃ¡lise de perfil avanÃ§ada","MemÃ³ria do Vortex","Suporte prioritÃ¡rio"],
    modelo:"Claude Haiku"
  },
  { id:"elite", nome:"Elite", emoji:"?", desc:"Sem preocupaÃ§Ã£o",
    creditos:8000, imgs:800, vids:120, voz:true,
    promo:97, normal:199, cor:"#f472b6", anim:"portal", animLabel:"? Portal CÃ³smico",
    preco_mensal:97, preco_anual:80, creditos_anual:10000,
    features:["8.000 crÃ©ditos/mÃªs","800 imagens IA","120 vÃ­deos","Clonagem de voz premium","Chat com Claude Sonnet","Todos os recursos","API prÃ³pria (em breve)","Suporte VIP 24h"],
...
const PACKS = [
  {
    id:"basico", nome:"Pack BÃ¡sico", emoji:"?", creditos:500,
    promo:15, normal:19, cor:"#fbbf24",
    desc:"Ideal para testar mais â€” sem prazo de validade",
    features:["500 crÃ©ditos extras","Sem expiraÃ§Ã£o","~50 roteiros ou 100 imagens"],
  },
  {
    id:"pro", nome:"Pack Pro", emoji:"??", creditos:2000,
    promo:45, normal:59, cor:"#9d5cff", popular:true,
    desc:"Melhor custo-benefÃ­cio â€” 22% mais barato por crÃ©dito",
    features:["2.000 crÃ©ditos extras","Sem expiraÃ§Ã£o","~200 roteiros ou 400 imagens"],
  },
  {
    id:"max", nome:"Pack Max", emoji:"??", creditos:5000,
    promo:89, normal:119, cor:"#f472b6",
    desc:"Para quem cria muito â€” 40% mais barato por crÃ©dito",
    features:["5.000 crÃ©ditos extras","Sem expiraÃ§Ã£o","~500 roteiros ou 1.000 imagens"],
  },
];
...
function GeradorImagens() {
  const ESTILOS = [
    ...
    { id:"cinematico", label:"?? CinematogrÃ¡fico", prompt:"cinematic, movie scene, dramatic lighting" },
  ...
  const MODELOS_IMG = [
    // GrÃ¡tis
    { id:"pollinations",   nome:"?? Pollinations",      desc:"Ilimitado â€¢ GrÃ¡tis",     creditos:0, cor:"#10B981", endpoint:"pollinations", free:true, grupo:"GrÃ¡tis" },
    { id:"hf_flux",        nome:"?? FLUX Schnell HF",   desc:"HuggingFace â€¢ GrÃ¡tis",   creditos:0, cor:"#F59E0B", endpoint:"hf_flux",      free:true, grupo:"GrÃ¡tis" },
    { id:"gemini",         nome:"?? Nano Banana Pro",   desc:"Google â€¢ GrÃ¡tis",        creditos:0, cor:"#4285F4", endpoint:"gemini",       free:true, grupo:"GrÃ¡tis" },
    // AIML â€” usa crÃ©ditos da sua key
    { id:"aiml_flux_schnell",nome:"? FLUX Schnell AIML", desc:"AIML â€¢ RÃ¡pido",        creditos:5, cor:"#7b2fff", endpoint:"aiml_flux",    aiml:true, grupo:"AIML" },
    { id:"aiml_flux_dev",    nome:"?? FLUX Dev AIML",    desc:"AIML â€¢ Alta qualidade", creditos:10, cor:"#9d5cff", endpoint:"aiml_flux_dev",aiml:true, grupo:"AIML" },
    { id:"aiml_gpt_image",   nome:"??? GPT Image 1.5",   desc:"AIML â€¢ OpenAI premium", creditos:10, cor:"#f472b6", endpoint:"aiml_gpt",     aiml:true, grupo:"AIML" },
    // FAL premium â€” crÃ©ditos calculados com margem de seguranÃ§a real
    { id:"flux_dev",  nome:"Flux Dev",        desc:"FAL â€¢ Thumbnails",      creditos:10,  cor:"#7C3AED", endpoint:"flux-dev",  grupo:"FAL", plano_min:"creator" },
    { id:"ideogram",  nome:"Ideogram v2 ?",  desc:"FAL â€¢ Texto em imagem", creditos:25, cor:"#F59E0B", endpoint:"ideogram",  grupo:"FAL", plano_min:"pro" },
    { id:"stability", nome:"Stability Ultra", desc:"FAL â€¢ Ultra realista",  creditos:20, cor:"#EF4444", endpoint:"stability", grupo:"FAL", plano_min:"pro" },
  ];
    ...
    <div className="page">
      ...
      <div style={{marginBottom:"1rem"}}>
        <div style={{fontSize:12,color:"var(--text3)",marginBottom:8,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            ...
            Gerado com Vortex â€” clique para copiar o prompt
          </div>
          <span style={{fontSize:10,color:"#7C3AED",fontWeight:600}}>? 8 exemplos</span>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:6}}>
          {[
            {img:"https://picsum.photos/seed/terror1/300/200",tag:"Terror",tagCor:"#ef4444",titulo:"A Noite Que Mudou Tudo",sub:"True crime â€¢ 2.3M views",prompt:"homem sozinho em corredor escuro, sombra ameaÃ§adora, terror psicolÃ³gico, cinematogrÃ¡fico, thumbnail viral"},
            {img:"https://picsum.photos/seed/gaming2/300/200",tag:"Gaming",tagCor:"#3b82f6",titulo:"Level ImpossÃ­vel",sub:"FPS gameplay â€¢ 4.1M views",prompt:"personagem de game em batalha Ã©pica, explosÃµes neon, thumbnail viral gaming TikTok"},
            {img:"https://picsum.photos/seed/crypto3/300/200",tag:"FinanÃ§as",tagCor:"#f59e0b",titulo:"R$50k em 30 Dias",sub:"Crypto â€¢ 1.8M views",prompt:"homem surpreso com grÃ¡fico de crypto explodindo, dinheiro, thumbnail viral finanÃ§as"},
            {img:"https://picsum.photos/seed/sunrise4/300/200",tag:"Lifestyle",tagCor:"#10b981",titulo:"Acordei Ã s 4h por 30d",sub:"Motivacional â€¢ 3.5M views",prompt:"silhueta dramÃ¡tica ao nascer do sol, cÃ©u laranja e roxo, motivacional, thumbnail viral"},
            {img:"https://picsum.photos/seed/anime5/300/200",tag:"Anime",tagCor:"#ec4899",titulo:"Protagonista Ã‰pico",sub:"Edit â€¢ 5.2M views",prompt:"personagem anime em pose Ã©pica com aura de poder, estilo Jujutsu Kaisen, thumbnail viral"},
            {img:"https://picsum.photos/seed/dark6/300/200",tag:"Dark Art",tagCor:"#6b7280",titulo:"Segredo Sombrio",sub:"MistÃ©rio â€¢ 2.8M views",prompt:"rosto misterioso emergindo das sombras, luz dramÃ¡tica, atmosfera dark cinematogrÃ¡fica"},
            {img:"https://picsum.photos/seed/tech7/300/200",tag:"Tech",tagCor:"#06b6d4",titulo:"IA vai te substituir",sub:"Tecnologia â€¢ 6.1M views",prompt:"robÃ´ futurista com IA, cÃ³digo hologrÃ¡fico, estÃ©tica cyberpunk, thumbnail viral tech"},
            {img:"https://picsum.photos/seed/fitness8/300/200",tag:"Fitness",tagCor:"#f97316",titulo:"TransformaÃ§Ã£o 90 dias",sub:"Fitness â€¢ 4.4M views",prompt:"transformaÃ§Ã£o fÃ­sica dramÃ¡tica, luz intensa de academia, silhueta musculosa, thumbnail fitness"},
          ].map((ex,i)=>(
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(130px,1fr))",gap:8}}>
          {MODELOS_IMG.map(m=>(
            <button key={m.id} onClick={()=>setModelo(m)}
              ...
              <span style={{fontSize:11,color:"var(--text3)",fontWeight:600}}>?? {m.creditos} crÃ©ditos</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="card" style={{marginBottom:"1rem"}}>
        <div className="field">
          ...
          <textarea className="input" rows={4}
            placeholder="Ex: homem olhando para explosÃ£o de luz, perspectiva dramÃ¡tica, cores vibrantes, thumbnail TikTok..."
        ...
        <div style={{background:"#7C3AED12",border:"1px solid #7C3AED30",borderRadius:8,padding:"8px 12px",marginBottom:12,display:"flex",justifyContent:"space-between"}}>
          <span style={{fontSize:13,color:"#7C3AED",fontWeight:600}}>{modelo.nome}{estilo?` â€¢ ${estilo.label}`:""}</span>
          <span style={{fontSize:13,color:"var(--text2)"}}>?? {modelo.creditos} crÃ©ditos</span>
        </div>
        ...
        {modelo.creditos>0&&(
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
            ...
            <span style={{fontSize:12,color:"var(--text3)"}}>?? Custo: <strong style={{color:"var(--text2)"}}>{modelo.creditos} crÃ©ditos</strong></span>
            <span style={{fontSize:11,color:"var(--text3)"}}>{"Ëœ R$"+(modelo.creditos*0.023).toFixed(2)}</span>
          </div>
        )}
      ...
      {resultado&&(
        <div className="card">
          ...
          <div style={{fontSize:11,color:"var(--text3)",textAlign:"center",marginBottom:4}}>
            ?? {modeloAtual} â€¢ <a href={resultado} target="_blank" rel="noreferrer" style={{color:"#7C3AED"}}>abrir em tela cheia</a>
          </div>
          <div style={{display:"flex",gap:8,justifyContent:"center",marginTop:8}}>
            <button onClick={()=>{
              ...
              alert("? Vortex aprendeu! PrÃ³ximas imagens serÃ£o ainda melhores.");
...
// ------------------------------------------------------
// EDITOR VISUAL DE THUMBNAIL â€” Fabric.js no browser
// ------------------------------------------------------
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.95)",zIndex:1000,display:"flex",flexDirection:"column",overflow:"auto"}}>
      ...
      <div style={{display:"flex",flex:1,overflow:"auto",gap:0}}>
        ...
        <div style={{width:280,background:"#0a0015",borderLeft:"1px solid rgba(124,92,252,.2)",padding:"12px",overflow:"auto",flexShrink:0}}>
          ...
          {/* Texto principal */}
          <div style={{marginBottom:12}}>
            <label style={{fontSize:11,color:"var(--text3)",display:"block",marginBottom:4}}>TÃ�TULO</label>
          ...
          {/* SubtÃ­tulo */}
          <div style={{marginBottom:12}}>
            <label style={{fontSize:11,color:"var(--text3)",display:"block",marginBottom:4}}>SUBTÃ�TULO</label>
...
// --------------------------------------------------------------
// VORTEX STUDIO â€” Pipeline completo de vÃ­deo IA
// Claude ? ElevenLabs ? FLUX ? Kling = VÃ­deo Ãºnico Vortex
// --------------------------------------------------------------
function VortexStudio() {
  ...
  const ESTILOS = [
    { id:"cinematografico", nome:"?? CinematogrÃ¡fico", desc:"Dark e dramÃ¡tico" },
    { id:"viral",           nome:"?? Viral",           desc:"Vibrante e dinÃ¢mico" },
  ...
  const MOTORES = [
    { id:"wan22_fast",  nome:"? RÃ¡pido",      creditos:60,  plano:"starter", desc:"GeraÃ§Ã£o em segundos" },
    { id:"kling3_std",  nome:"?? PadrÃ£o",      creditos:90,  plano:"creator", desc:"Alta qualidade 4K" },
    ...
    { id:"veo31_fast",  nome:"?? Ultra",       creditos:185, plano:"elite",   desc:"MÃ¡xima qualidade possÃ­vel" },
  ];

  // Etapas sem mencionar APIs
  const ETAPAS = [
    { icon:"??", nome:"Criando roteiro",      desc:"Desenvolvendo narrativa e cenas..." },
    { icon:"???", nome:"Gerando narraÃ§Ã£o",     desc:"Sintetizando voz profissional..." },
    { icon:"??", nome:"Renderizando cena",    desc:"Construindo identidade visual..." },
    { icon:"??", nome:"Animando vÃ­deo",       desc:"Dando vida Ã  cena final..." },
  ];

  const motorAtual = MOTORES.find(m => m.id === modelo) || MOTORES[1];

  async function criar() {
    if(!tema.trim()) { setErro("Descreva o tema do seu vÃ­deo"); return; }
    setLoading(true);
    setErro("");
    setResultado(null);
    setEtapaAtual(1);
    ...
    try {
      ...
      clearInterval(timer);
      setErro(e.message || "Erro ao criar vÃ­deo. Tente novamente.");
      setEtapaAtual(0);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="page">
      ...
      <div className="page-header">
        ...
        <p className="page-sub">Descreva sua ideia. O Vortex cria o vÃ­deo completo â€” roteiro, voz, imagem e movimento.</p>
      </div>

      {/* Input principal â€” grande e limpo */}
      {!loading && !resultado && (
        <div className="card" style={{marginBottom:12}}>
          <label className="label" style={{fontSize:13}}>Qual Ã© a ideia do seu vÃ­deo?</label>
          <textarea className="input" rows={3}
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
            <div>
              ...
              <select className="input" value={nicho} onChange={e=>setNicho(e.target.value)}>
                {["geral","terror","gaming","finanÃ§as","fitness","tech","lifestyle","anime","motivacional"].map(n=>(
            ...
            <div>
              ...
              <select className="input" value={modelo} onChange={e=>setModelo(e.target.value)}>
                {MOTORES.map(m=>(
                  <option key={m.id} value={m.id}>{m.nome} â€” {m.creditos}cr</option>
                ))}
              </select>
            </div>
          </div>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",
            ...
            <span style={{fontSize:12,color:"var(--text3)"}}>
              ?? <strong style={{color:"var(--text)"}}>{motorAtual.creditos} crÃ©ditos</strong> Â· {motorAtual.desc}
            </span>
          ...
          <button className="btn btn-full" onClick={criar} disabled={!tema.trim()}
            ...
            ?? Criar VÃ­deo â€” {motorAtual.creditos} crÃ©ditos
          </button>
        </div>
      )}
      ...
      {loading && (
        <div className="card" style={{textAlign:"center",padding:"2rem 1rem"}}>
          ...
          <div style={{fontFamily:"var(--fh)",fontSize:20,fontWeight:800,
            ...
            Vortex Studio estÃ¡ criando...
          </div>
          <div style={{fontSize:13,color:"var(--text3)",marginBottom:24}}>
            Isso pode levar atÃ© 1 minuto. NÃ£o feche essa tela.
          </div>
...
function GeradorVideos() {
  // MODELOS de vÃ­deo
  const MODELOS_VIDEO = [
    { id:"studio",     nome:"? Vortex Studio",  tag:"Exclusivo",       creditos:90, cor:"#9333ea", api:"studio", plano_min:"creator", desc:"Pipeline completo â€” roteiro, voz, imagem e vÃ­deo", aviso:null, studio:true },
    { id:"hf_ltx",     nome:"?? LTX-Video",      tag:"GrÃ¡tis",          creditos:0,  cor:"#F59E0B", api:"hf",  free:true,   plano_min:"free",    desc:"HuggingFace â€¢ Open source",         aviso:null },
    { id:"wan26_fast", nome:"? WAN 2.6 Fast",    tag:"Open source",     creditos:5,  cor:"#7C3AED", api:"fal", fal_model:"fal-ai/wan/t2v-1.3b",               plano_min:"starter", desc:"Alibaba â€¢ RÃ¡pido",           aviso:null },
    { id:"hailuo23",   nome:"?? Hailuo 2.3",      tag:"Custo-benefÃ­cio", creditos:10, cor:"#8B5CF6", api:"fal", fal_model:"fal-ai/minimax/video-01-live",       plano_min:"starter", desc:"MiniMax â€¢ menos de 30s",    aviso:null },
    { id:"hunyuan_vid",nome:"?? Hunyuan Video",   tag:"Open source",     creditos:10, cor:"#EF4444", api:"fal", fal_model:"fal-ai/hunyuan-video",              plano_min:"starter", desc:"Tencent â€¢ 1080p",           aviso:null },
    { id:"wan26_hd",   nome:"?? WAN 2.6 HD",      tag:"Alta qualidade",  creditos:15, cor:"#7C3AED", api:"fal", fal_model:"fal-ai/wan/t2v-5b-720p",            plano_min:"creator", desc:"Alibaba â€¢ 720p",            aviso:null },
    { id:"seedance2",  nome:"?? Seedance 2.0",     tag:"#1 sem Ã¡udio",   creditos:15, cor:"#F59E0B", api:"fal", fal_model:"fal-ai/bytedance/seedance-1-lite",  plano_min:"creator", desc:"ByteDance â€¢ Top movement",  aviso:null },
    { id:"minimax_vid",nome:"?? Minimax Video",    tag:"Personagens",     creditos:15, cor:"#EC4899", api:"fal", fal_model:"fal-ai/minimax-video/video-01",     plano_min:"creator", desc:"MiniMax â€¢ Personagens",     aviso:null },
    { id:"kling3_std", nome:"?? Kling 3.0",        tag:"4K â€¢ 60fps",      creditos:20, cor:"#0EA5E9", api:"fal", fal_model:"fal-ai/kling-video/v1.6/standard/text-to-video", plano_min:"creator", desc:"Kuaishou â€¢ 4K",  aviso:null },
    { id:"luma_dream", nome:"?? Luma Dream",       tag:"Surreal",         creditos:20, cor:"#10B981", api:"fal", fal_model:"fal-ai/luma-dream-machine",         plano_min:"creator", desc:"Luma AI â€¢ Cenas surreais",  aviso:null },
    { id:"happyhorse", nome:"?? HappyHorse 1.0",   tag:"#1 mundial",      creditos:30, cor:"#f472b6", api:"fal", fal_model:"fal-ai/alibaba/happyhorse",         plano_min:"pro",     desc:"Alibaba â€¢ lip-sync 7 idiomas", aviso:null },
    { id:"luma_ray3",  nome:"?? Luma Ray 3",       tag:"HDR 16-bit",      creditos:35, cor:"#10B981", api:"fal", fal_model:"fal-ai/luma-dream-machine/ray-3",   plano_min:"pro",     desc:"Luma AI â€¢ Primeiro HDR",    aviso:null },
    { id:"kling3_pro", nome:"?? Kling 3.0 Pro",    tag:"Top â€¢ 4K 60fps",  creditos:40, cor:"#F59E0B", api:"fal", fal_model:"fal-ai/kling-video/v1.6/pro/text-to-video", plano_min:"pro",  desc:"Kuaishou â€¢ AI Director",  aviso:null },
    { id:"runway_gen4",nome:"?? Runway Gen-4.5",   tag:"Melhor controle", creditos:35, cor:"#74AA9C", api:"fal", fal_model:"fal-ai/runway/gen-4",               plano_min:"pro",     desc:"Runway â€¢ Motion brushes",   aviso:null },
    { id:"veo31_lite", nome:"? Veo 3.1 Lite",     tag:"Google",          creditos:15, cor:"#9d5cff", api:"fal", fal_model:"fal-ai/veo3/lite",                  plano_min:"pro",     desc:"Google â€¢ Ã¡udio nativo",     aviso:null },
    { id:"sora2",      nome:"?? Sora 2 Pro",       tag:"OpenAI",          creditos:120,cor:"#74AA9C", api:"fal", fal_model:"fal-ai/sora",                       plano_min:"elite",   desc:"OpenAI â€¢ API atÃ© set/2026", aviso:"?? Consome 120 crÃ©ditos por vÃ­deo." },
    { id:"veo31_fast", nome:"?? Veo 3.1 Fast",     tag:"Google Premium",  creditos:160,cor:"#EF4444", api:"fal", fal_model:"fal-ai/veo3/fast",                  plano_min:"elite",   desc:"Google â€¢ 4K + Ã¡udio",       aviso:"?? Consome 160 crÃ©ditos por vÃ­deo." },
    { id:"veo31",      nome:"? Veo 3.1 Quality",  tag:"Melhor do mundo", creditos:325,cor:"#f72faa", api:"fal", fal_model:"fal-ai/veo3",                       plano_min:"elite",   desc:"Google â€¢ MÃ¡xima qualidade", aviso:"?? Consome 325 crÃ©ditos por vÃ­deo." },
  ];
  ...
  const gerar = async () => {
    if(!prompt.trim()) return;
    setLoading(true); setErro(""); setResultado(null); setProgresso(0);
    const msgs = ["?? Enviando prompt...","?? IA gerando vÃ­deo...","?? Renderizando...","?? Finalizando..."];
    ...
    try {
      // Se for Vortex Studio â€” pipeline completo
      if(modelo.studio) {
        const res = await fetch(`${BACKEND}/vortex-studio/criar`, {
      ...
      clearInterval(tick);
      setErro(e.message || "Erro ao gerar vÃ­deo");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div className="eyebrow"><div className="eyebrow-dot"/>IA & CriaÃ§Ã£o</div>
        <h1 className="page-title">Gerador de <em>VÃ­deo</em></h1>
      ...
      <div>
          ...
          <div className="card" style={{marginBottom:12}}>
            ...
            <textarea className="input" rows={3} value={prompt}
              ...
              placeholder="Ex: Uma floresta densa Ã  noite, neblina surgindo entre as Ã¡rvores..."
            ...
            {modelo.creditos>0 && (
              <div style={{display:"flex",justifyContent:"space-between",padding:"8px 12px",
                ...
                <span style={{fontSize:12,color:"var(--text3)"}}>
                  {"?? Custo: "}<strong style={{color:"var(--text)"}}>{modelo.creditos+" crÃ©ditos"}</strong>
                </span>
                <span style={{fontSize:11,color:modelo.creditos>50?"#fbbf24":"var(--text3)"}}>
                  {modelo.creditos>100?"?? Alto consumo":modelo.creditos>30?"?? Moderado":"? EconÃ´mico"}
                </span>
              </div>
            )}
            ...
            {!loading && (
              <button className="btn btn-full" onClick={gerar} disabled={!prompt.trim()}
                ...
                {"?? Gerar VÃ­deo â€” "+modelo.creditos+" crÃ©ditos"}
              </button>
            )}
          ...
          {resultado && (
            <div className="card">
              <div style={{fontWeight:700,marginBottom:12,color:"#7C3AED"}}>{"? VÃ­deo gerado!"}</div>
...
function ClonadorVoz() {
  const VOZES=[
    {id:"onwK4e9ZLuTAKqWW03F9",nome:"VORTEX",icon:"??",desc:"Voz oficial do Vortex â€” dramÃ¡tica e envolvente",estilo:"Ã‰pico"},
    {id:"pNInz6obpgDQGcFmaJgB",nome:"Adam",icon:"???",desc:"Grave e autoritÃ¡rio â€” ideal para terror e suspense",estilo:"Grave"},
    {id:"TX3LPaxmHKxFdv7VOQHJ",nome:"Liam",icon:"?",desc:"Jovem e energÃ©tico â€” perfeito para gaming",estilo:"EnergÃ©tico"},
    {id:"ErXwobaYiN019PkySvjV",nome:"Antoni",icon:"??",desc:"Tom suave e narrativo â€” Ã³timo para storytelling",estilo:"Narrativo"},
    {id:"VR6AewLTigWG4xSOukaG",nome:"Arnold",icon:"??",desc:"Imponente e confiante â€” autoridade mÃ¡xima",estilo:"Imponente"},
    {id:"EXAVITQu4vr4xnSDxMaL",nome:"Bella",icon:"??",desc:"Feminina e carismÃ¡tica â€” engajamento alto",estilo:"CarismÃ¡tico"},
  ];
  ...
  return(
    <div className="page">
      <div className="page-header">
        <div className="eyebrow"><div className="eyebrow-dot"/>IA & CriaÃ§Ã£o</div>
        <h1 className="page-title">NarraÃ§Ã£o com IA</h1>
        <p className="page-sub">Transforme seu roteiro em narraÃ§Ã£o profissional</p>
      </div>

      {/* Player de demo de narraÃ§Ã£o */}
      <div style={{marginBottom:"1rem"}}>
        <div style={{fontSize:12,color:"var(--text3)",marginBottom:8,display:"flex",alignItems:"center",gap:6}}>
          <span style={{width:6,height:6,borderRadius:"50%",background:"#7C3AED",display:"inline-block"}}/>
          Estilos de narraÃ§Ã£o â€” clique para usar o texto
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
          {[
            {icon:"??",cor:"#ef4444",bg:"linear-gradient(135deg,#1a0000,#3d0000)",titulo:"Terror & Suspense",sub:"Voz grave e dramÃ¡tica",exemplo:"NinguÃ©m sabia o que estava prestes a acontecer naquela noite. O silÃªncio era ensurdecedor..."},
            {icon:"?",cor:"#3b82f6",bg:"linear-gradient(135deg,#00051a,#001040)",titulo:"Gaming Hype",sub:"EnergÃ©tico e rÃ¡pido",exemplo:"Esse glitch quebrou o jogo INTEIRO! NinguÃ©m consegue passar desse nÃ­vel... atÃ© agora!"},
            {icon:"??",cor:"#f59e0b",bg:"linear-gradient(135deg,#1a0d00,#3d1f00)",titulo:"FinanÃ§as",sub:"AutoritÃ¡rio e confiante",exemplo:"Esse ativo vai triplicar em 2025. Aqui estÃ¡ o motivo que ninguÃ©m estÃ¡ te contando..."},
            {icon:"??",cor:"#a855f7",bg:"linear-gradient(135deg,#0d0020,#2d0057)",titulo:"Storytelling",sub:"Narrativo e envolvente",exemplo:"Era uma noite fria quando tudo mudou para sempre. Eu nÃ£o sabia que aquela seria minha Ãºltima chance..."},
          ].map((d,i)=>(
      ...
      <div className="card" style={{marginBottom:"1rem"}}>
        ...
        <div style={{background:"#7C3AED12",border:"1px solid #7C3AED30",borderRadius:8,padding:"8px 12px",marginBottom:12,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <span style={{fontSize:13,color:"#7C3AED",fontWeight:600}}>{voz.icon} {voz.nome} â€¢ {voz.estilo}</span>
          <span style={{fontSize:13,color:"var(--text2)"}}>?? {Math.ceil(texto.length/100)} crÃ©ditos</span>
        </div>
      ...
      {resultado&&(
        <div className="card">
          <div style={{fontWeight:700,marginBottom:16,color:"#7C3AED"}}>? NarraÃ§Ã£o pronta!</div>
...
function GeradorMusica() {
  const ESTILOS=[
    {id:"dark_electronic",label:"?? Dark Electronic",desc:"TensÃ£o â€” terror/thriller",provider:"elevenlabs"},
    {id:"cinematic",label:"?? CinematogrÃ¡fico",desc:"Ã‰pico â€” trailers",provider:"elevenlabs"},
    {id:"lofi_chill",label:"?? Lo-fi Chill",desc:"Relaxante â€” lifestyle",provider:"elevenlabs"},
    {id:"hype_trap",label:"? Hype Trap",desc:"Energia â€” gaming",provider:"elevenlabs"},
    {id:"ambient",label:"?? Ambient",desc:"AtmosfÃ©rico â€” ASMR",provider:"elevenlabs"},
    {id:"horror",label:"?? Horror",desc:"Assustador â€” true crime",provider:"elevenlabs"},
    {id:"suno",label:"?? Suno AI",desc:"MÃºsica completa com melodia",provider:"suno"},
    {id:"udio",label:"?? Udio",desc:"MÃºsica + voz cantada",provider:"udio"},
  ];
  ...
  return(
    <div className="page">
      <div className="page-header"><div className="eyebrow"><div className="eyebrow-dot"/>IA & CriaÃ§Ã£o</div><h1 className="page-title">Gerador de <em>MÃºsica</em></h1><p className="page-sub">Trilhas com ElevenLabs, Suno AI e Udio.</p></div>
      <div className="card" style={{marginBottom:"1rem"}}>
        ...
        <div className="field"><label className="label">DuraÃ§Ã£o</label><div className="chips">{[5,10,15,22].map(d=><button key={d} className={"chip "+(duracao===d?"active":"")} onClick={()=>setDuracao(d)} disabled={loading}>{d}s</button>)}</div></div>
        ...
        {erro&&<div style={{color:"#f87171",fontSize:13,marginBottom:8}}>?? {erro}</div>}
        <button className="btn btn-full" onClick={gerar} disabled={loading}>{loading?"? Compondo...":"?? Gerar MÃºsica â€” 3 crÃ©ditos"}</button>
      </div>
      {resultado&&(
      <div className="card">
        <div className="card-title" style={{marginBottom:12}}>? MÃºsica gerada!</div>
...
function ScoreViral() {
  ...
  const extrairScores=(txt)=>{
    ...
    const dims=[
      ...
      {key:"retencao",label:"RetenÃ§Ã£o",icon:"???",color:"#22d3ee"},
      {key:"emocao",label:"EmoÃ§Ã£o",icon:"??",color:"#f87171"},
      {key:"shares",label:"Shares",icon:"??",color:"#34d399"},
      {key:"comentario",label:"ComentÃ¡rios",icon:"??",color:"#fbbf24"},
    ];
    const scores=[];
    dims.forEach(d=>{
      const patterns=[
    ...
    const mediaMatch=txt.match(/mÃ©dia[^0-9]*(\d+(?:\.\d+)?)/i)||txt.match(/score[^0-9]*(\d+(?:\.\d+)?)/i)||txt.match(/MÃ‰DIA[^0-9]*(\d+(?:\.\d+)?)/i);
  ...
  return(
    <div className="page">
      <div className="page-header">
        ...
        <p className="page-sub">Analise o potencial do seu roteiro em 5 dimensÃµes.</p>
        <div style={{display:"inline-flex",alignItems:"center",gap:6,marginTop:6,
          background:"rgba(52,211,153,.1)",border:".5px solid rgba(52,211,153,.3)",
          borderRadius:20,padding:"4px 12px"}}>
          <span style={{fontSize:12}}>?</span>
          <span style={{fontSize:11,color:"#34d399",fontWeight:600}}>GrÃ¡tis e ilimitado â€” use sem crÃ©ditos</span>
        </div>
      </div>
      ...
      <div className="card" style={{marginBottom:"1rem"}}>
        ...
        <div style={{fontSize:11,color:"var(--text3)",marginBottom:10}}>
          {roteiro.length} caracteres Â· ~{Math.ceil(roteiro.split(" ").length/2)} seg de fala
        </div>
        {erro&&<div className="erro">?? {erro}</div>}
        <button className="btn btn-full" onClick={analisar} disabled={loading||!roteiro.trim()}>
          {loading?<><div className="spinner"/>Analisando com IA...</>:"? Analisar Score Viral â€” GrÃ¡tis 8"}
        </button>
      </div>
      ...
      {sc&&(
        <div>
          ...
          {sc.dims.length>0&&(
            <div className="card" style={{marginBottom:12}}>
              <div className="card-title" style={{marginBottom:12}}>?? 5 DimensÃµes</div>
              {sc.dims.map((d,i)=>(
          ...
          {/* Texto completo */}
          <div className="card" style={{marginBottom:12}}>
            <div className="card-title" style={{marginBottom:10}}>?? AnÃ¡lise completa</div>
          ...
          {!feedbackDado&&(
            <div className="card" style={{background:"rgba(123,47,255,.04)",border:".5px solid rgba(123,47,255,.15)"}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                <span style={{fontSize:12,color:"var(--text3)",flex:1}}>?? Esse score foi Ãºtil?</span>
              </div>
              <div style={{display:"flex",gap:8}}>
                <button onClick={()=>{
                ...
                }} className="btn" style={{flex:1,fontSize:13}}>?? Ãštil</button>
                <button onClick={()=>{
                  fetch(`${BACKEND}/cerebro/feedback`,{method:"POST",headers:{"Content-Type":"application/json"},
...
function MemoriaVortex() {
  ...
  const CAMPOS=[
    ...
    {key:"publico_alvo",label:"PÃºblico-alvo",icon:"??",placeholder:"Ex: jovens 18-25, gamers, mÃ£es empreendedoras"},
    {key:"formatos_favoritos",label:"Formatos favoritos",icon:"??",placeholder:"Ex: talking head, POV, trending sounds"},
    {key:"evitar",label:"O que evitar",icon:"??",placeholder:"Ex: palavrÃ£o, polÃ­tica, assuntos pesados"},
    {key:"cta_favorito",label:"CTA favorito",icon:"??",placeholder:"Ex: Me segue pra mais! Comenta X se concordar"},
    {key:"referencias",label:"ReferÃªncias/inspiraÃ§Ãµes",icon:"?",placeholder:"Ex: @fulano, Nubank, estilo Hormozi"},
  ];

  return(
    <div className="page">
      <div className="page-header">
        <div className="eyebrow"><div className="eyebrow-dot"/>PersonalizaÃ§Ã£o</div>
        <h1 className="page-title">MemÃ³ria do <em>Vortex</em></h1>
        <p className="page-sub">O Vortex aprende com vocÃª. Essas informaÃ§Ãµes ficam em todos os roteiros.</p>
      </div>
      ...
      <div className="card" style={{marginBottom:12,background:"rgba(123,47,255,.04)",border:".5px solid rgba(123,47,255,.15)"}}>
        <div style={{display:"flex",gap:12,alignItems:"center"}}>
          <div style={{width:44,height:44,borderRadius:12,background:"linear-gradient(135deg,#7b2fff,#9d5cff)",
            ...
            {perfil.nicho==="terror"?"??":perfil.nicho==="gaming"?"??":perfil.nicho==="finanÃ§as"?"??":"??"}
          </div>
          <div>
            <div style={{fontSize:14,fontWeight:700,color:"var(--text)"}}>{perfil.nome||"Criador Vortex"}</div>
            <div style={{fontSize:12,color:"var(--text3)"}}>
              {perfil.nicho||"nicho nÃ£o definido"} Â· {perfil.plataforma||"plataforma nÃ£o definida"}
            </div>
          </div>
          <div style={{marginLeft:"auto",fontSize:11,color:"#7b2fff"}}>
            {Object.keys(mem).length} memÃ³rias salvas
          </div>
        </div>
      </div>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:editando===c.key?10:0}}>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <div>
                ...
                {!mem[c.key]&&editando!==c.key&&(
                  <div style={{fontSize:11,color:"var(--text3)"}}>NÃ£o definido</div>
                )}
              </div>
            </div>
            <div style={{display:"flex",gap:6,flexShrink:0}}>
      ...
      <div style={{textAlign:"center",fontSize:12,color:"var(--text3)",padding:"8px 0"}}>
        ?? Essas memÃ³rias sÃ£o usadas automaticamente em roteiros, chat e anÃ¡lises.
      </div>
    </div>
  );
}
...
function MeusProjetos() {
  ...
  return(<div className="page"><div className="page-header"><div className="eyebrow"><div className="eyebrow-dot"/>meu espaÃ§o</div><h1 className="page-title">Meus <em>Projetos</em></h1><p className="page-sub">HistÃ³rico de criaÃ§Ãµes.</p></div>{historico.length===0&&<div className="card" style={{textAlign:"center",padding:"3rem"}}><div style={{fontSize:48,marginBottom:12}}>??</div><p style={{color:"var(--text3)"}}>Nenhum projeto ainda.</p></div>}{historico.filter(h=>h.tipo==="debito").reverse().map((h,i)=><div key={i} className="card" style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",marginBottom:6}}><span style={{fontSize:22,flexShrink:0}}>{ICONS[h.operacao?.split("_").slice(0,2).join("_")]||"??"}</span><div style={{flex:1}}><div style={{fontSize:13,fontWeight:500,color:"var(--text)",textTransform:"capitalize"}}>{(h.operacao||"").replace(/_/g," ")}</div><div style={{fontSize:11,color:"var(--text3)"}}>{new Date(h.timestamp).toLocaleString("pt-BR",{day:"2-digit",month:"2-digit",hour:"2-digit",minute:"2-digit"})}</div></div><div style={{fontSize:12,color:"#f87171",fontWeight:600,flexShrink:0}}>-{h.quantidade} ??</div></div>)}</div>);
}
...
const CURRENCIES = {
  ...
  EUR:{ symbol:"â‚¬",  code:"EUR", rate:0.18 },
};
...
function useCurrency() {
  // Moedas internacionais desativadas â€” sÃ³ BRL por enquanto
  const currency = "BRL";
  const setCurrency = () => {};
  ...
  return (
    <div className="page">
      <div className="page-header">
        ...
        <p className="page-sub">Todos os planos com crÃ©ditos que renovam todo mÃªs.</p>
      </div>
      ...
      {(preLaunch||isPromo)&&(
        <div className="card" style={{marginBottom:12,background:"rgba(123,47,255,.06)",border:".5px solid rgba(123,47,255,.2)",textAlign:"center"}}>
          <div style={{fontSize:11,color:"var(--text3)",marginBottom:6,letterSpacing:".1em"}}>
            {preLaunch?"?? LANÃ‡AMENTO EM":"? PREÃ‡O DE LANÃ‡AMENTO POR"}
          </div>
          ...
          {showPromo&&<div style={{fontSize:11,color:"#34d399",marginTop:6}}>?? PreÃ§o de lanÃ§amento â€” garanta agora e mantÃ©m para sempre!</div>}
        </div>
      )}
        ...
        <div style={{display:"flex",gap:6}}>
          {["mensal","creditos"].map(t=>(
            <button key={t} onClick={()=>setTab(t)} style={{padding:"7px 18px",borderRadius:99,border:`.5px solid ${tab===t?"rgba(123,47,255,.5)":"var(--border2)"}`,background:tab===t?"rgba(123,47,255,.15)":"transparent",color:tab===t?"var(--purple2)":"var(--text3)",fontFamily:"var(--fh)",fontSize:12,fontWeight:700,cursor:"pointer"}}>
              {t==="mensal"?"?? Mensal":"?? CrÃ©ditos Avulsos"}
            </button>
          ))}
        </div>
      </div>
        ...
        <div className="card" style={{marginBottom:12,background:"rgba(52,211,153,.04)",border:".5px solid rgba(52,211,153,.2)"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              ...
              <div>
                <div style={{fontSize:14,fontWeight:700,color:"var(--text)"}}>Plano Free â€” R$0/mÃªs</div>
                <div style={{fontSize:11,color:"var(--text3)"}}>50 crÃ©ditos â€¢ ?? TendÃªncias ilimitado â€¢ ? Score ilimitado â€¢ 3 roteiros/dia</div>
              </div>
            </div>
          ...
          {planos.map(p=>{
            ...
            <div key={p.id} onClick={()=>setSel(p.id)} style={{background:sel===p.id?p.cor+"18":"var(--bg3)",border:`1.5px solid ${sel===p.id?p.cor:"var(--border)"}`,borderRadius:"var(--r-lg)",padding:"1.25rem 1rem",cursor:"pointer",transition:"all .2s",position:"relative"}}>
              ...
              <div style={{display:"flex",alignItems:"baseline",gap:6,marginBottom:4}}>
                ...
                <span style={{fontSize:11,color:"var(--text3)"}}>/mÃªs</span>
              </div>
              {showPromo&&<div style={{fontSize:10,color:"#34d399",marginBottom:4}}>?? {Math.round((1-preco(p)/p.normal)*100)}% off lanÃ§amento</div>}
              <div style={{fontSize:12,color:"var(--text2)",marginBottom:8,fontWeight:600}}>?? {creditos_tab(p).toLocaleString()} crÃ©ditos/mÃªs</div>
              {p.features&&sel===p.id&&(
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:10,marginBottom:"1.5rem"}}>
          {PACKS.map(p=>(
            <div key={p.id} onClick={()=>setSel("pack_"+p.id)} style={{background:sel==="pack_"+p.id?"rgba(123,47,255,.08)":"var(--bg3)",border:sel==="pack_"+p.id?"1px solid rgba(123,47,255,.5)":".5px solid var(--border)",borderRadius:"var(--r-lg)",padding:"1.25rem 1rem",cursor:"pointer",transition:"all .2s",position:"relative"}}>
              ...
              <div style={{fontSize:14,fontWeight:700,color:p.cor||"var(--accent)"}}>?? {p.creditos.toLocaleString()} crÃ©ditos</div>
              <div style={{fontSize:10,color:"var(--text3)",marginTop:2}}>NÃ£o expiram Â· Uso livre</div>
            ...
            return(
              <>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:13,padding:"7px 0",borderBottom:".5px solid var(--border)"}}>
                  <span>{item.nome}</span>
                  <span style={{fontWeight:700}}>{fmt(valor)}{!isPack?"/mÃªs":""}</span>
                </div>
...
function Configuracoes() {
  ...
  const IDIOMAS=["PortuguÃªs","InglÃªs","Espanhol"];

  return(
    <div className="page">
      <div className="page-header">
        <div className="eyebrow"><div className="eyebrow-dot"/>Sistema</div>
        <h1 className="page-title">Configu<em>raÃ§Ãµes</em></h1>
        <p className="page-sub">Personalize o Vortex para o seu perfil.</p>
      </div>

      {/* Abas */}
      <div style={{display:"flex",gap:6,marginBottom:16,background:"rgba(255,255,255,.04)",borderRadius:12,padding:4}}>
        {[{id:"perfil",icon:"??",label:"Perfil"},{id:"preferencias",icon:"??",label:"PreferÃªncias"},{id:"funcoes",icon:"??",label:"FunÃ§Ãµes"}].map(t=>(
      ...
      {aba==="perfil"&&(
        <div className="card" style={{marginBottom:12}}>
          ...
          <div className="field">
            ...
            <input className="input" placeholder="Ex: JoÃ£o, @joaocriador" value={perfil.nome||""} onChange={e=>setPer("nome",e.target.value)}/>
          </div>
          ...
          <div className="field">
            <label className="label">Idioma dos conteÃºdos</label>
            <div className="chips">
          ...
          <div className="field">
            <label className="label">Tom de voz do conteÃºdo</label>
            <div className="chips">
              {["Informal","Profissional","HumorÃ­stico","DramÃ¡tico","Educativo"].map(t=>(
      ...
      {aba==="preferencias"&&(
        <div className="card" style={{marginBottom:12}}>
          <div className="card-title" style={{marginBottom:14}}>?? PreferÃªncias do app</div>

          <div className="field">
            <label className="label">DuraÃ§Ã£o padrÃ£o do roteiro</label>
            <div className="chips">
          ...
          <div className="field">
            <label className="label">Modelo padrÃ£o de chat</label>
            <div className="chips">
              {["Auto (melhor disponÃ­vel)","Claude","GPT-4o","Gemini","Groq (rÃ¡pido)"].map(m=>(
          ...
          <div className="field">
            <label className="label">PaÃ­s para tendÃªncias</label>
            <div className="chips">
              {["???? Brasil","???? EUA","???? Portugal","???? MÃ©xico"].map(p=>(
      ...
      {aba==="funcoes"&&(
        <div className="card" style={{marginBottom:12}}>
          <div className="card-title" style={{marginBottom:14}}>?? FunÃ§Ãµes ativas</div>
          {[
            {key:"analise_perfil",name:"AnÃ¡lise de Perfil",desc:"Analisa perfis de redes sociais",icon:"??"},
            {key:"roteiro",name:"Gerador de Roteiro",desc:"Roteiros virais com IA",icon:"??"},
            {key:"tendencias",name:"TendÃªncias",desc:"Google Trends em tempo real",icon:"??"},
            {key:"gerador_imagem",name:"Imagens IA",desc:"FLUX, DALL-E e mais",icon:"??"},
            {key:"gerador_video",name:"VÃ­deos IA",desc:"DisponÃ­vel em julho",icon:"??"},
            {key:"clonador_voz",name:"Clonar Voz",desc:"ElevenLabs voice cloning",icon:"???"},
            {key:"musica",name:"Gerar MÃºsica",desc:"Trilhas para vÃ­deos",icon:"??"},
      ...
      <button className="btn btn-full" onClick={salvar}>?? Salvar configuraÃ§Ãµes</button>
    </div>
  );
}
...
function WelcomeScreen({ onEnter }) {
  ...
  useEffect(() => {
    ...
    // Canvas sÃ³ com partÃ­culas sobre a imagem â€” sem buraco negro desenhado
    ...
    const draw=()=>{
      ...
      // PartÃ­culas saindo do centro (buraco negro na imagem)
      stars.forEach(s=>{
        const tw=s.a*(.3+.7*Math.sin(t*s.speed+s.phase));
        cx.beginPath();cx.arc(s.x,s.y,s.r,0,Math.PI*2);
        cx.fillStyle=s.color;cx.globalAlpha=tw*.7;cx.fill();
        // Move partÃ­culas para fora do centro (efeito vÃ³rtice)
  ...
  useEffect(()=>{
    const LINES=[
      {h:"Crie conteÃºdo\nque viraliza.",s:"Roteiros, hooks e tendÃªncias com IA."},

      {h:"Do nada ao vÃ­deo\nem segundos.",s:"Cole o tema â€” o Vortex faz o resto."},

      {h:"Trends antes\nde todos.",s:"Fique um passo Ã  frente do algoritmo."},

      {h:"Seu criativo.\nNossa inteligÃªncia.",s:"EstÃºdio de IA para criadores brasileiros."},
    ...
    <div style={{position:"fixed",inset:0,zIndex:300,background:"#000",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",overflow:"hidden"}}>
      ...
      <div style={{position:"relative",zIndex:20,display:"flex",flexDirection:"column",alignItems:"center",textAlign:"center",padding:"0 24px",marginTop:220}}>
        <div style={{fontFamily:"'Syne',sans-serif",fontSize:10,fontWeight:700,letterSpacing:".22em",textTransform:"uppercase",color:"#4a4470",marginBottom:12}}>
          VOR<span style={{color:"#9d5cff",textShadow:"0 0 12px rgba(123,47,255,.8)"}}>TEX</span> Â· AI Creator Studio
        </div>
...
function Onboarding({ onConcluir }) {
  ...
  async function concluir(){
    ...
    } catch { setBoasVindas("Pronto! Vamos criar conteÃºdo viral! ??"); }
  ...
  return(
    <div className="ob-overlay"><div className="ob-box">
      ...
      {step===3&&<div className="ob-fim">
        ...
        {exemploHook&&<div style={{background:"rgba(123,47,255,.1)",border:".5px solid rgba(123,47,255,.3)",borderRadius:12,padding:"12px 14px",margin:"12px 0",textAlign:"left"}}><div style={{fontSize:10,color:"var(--accent)",fontWeight:700,marginBottom:6,letterSpacing:1}}>? PREVIEW â€” HOOK VIRAL</div><div style={{fontSize:14,fontWeight:700,color:"var(--text)",lineHeight:1.5,fontStyle:"italic"}}>"{exemploHook}"</div></div>}
        <div style={{fontSize:12,color:"var(--text3)",margin:".5rem 0 1.25rem"}}>{dados.nicho&&<span>Nicho: <strong style={{color:"var(--purple2)"}}>{dados.nicho}</strong></span>}{dados.plataformas.length>0&&<span> Â· {dados.plataformas.join(", ")}</span>}</div>
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    const verificarAuth=async()=>{
      try{
        ...
        if(params.get("pagamento")==="ok"){window.history.replaceState({},"",window.location.pathname);setTimeout(()=>alert("? Pagamento confirmado! CrÃ©ditos adicionados."),1000);}
        const r=await fetch(`${BACKEND}/auth/me`).catch(()=>null);
        if(r&&r.ok){
          const d=await r.json();
          setUsuario(d.usuario);
          setSaldo(d.usuario.creditos||0);
          setScreen("app"); // jÃ¡ logado ? pular welcome
        }
      }catch{}
  ...
  // Modal de login â€” aparece quando usuÃ¡rio tenta usar feature paga
  const [showLogin, setShowLogin] = useState(false);
  const [loginMsg, setLoginMsg] = useState("");

  // FunÃ§Ã£o global para pedir login
  window.__vortexPedirLogin = (msg="FaÃ§a login para continuar") => {
    setLoginMsg(msg);
    setShowLogin(true);
  };
  ...
  return(
    <>
      ...
      {/* Modal de Login â€” aparece sobre qualquer tela */}
      {showLogin && (
          ...
          <main className="main">
            <header className="topbar">
              ...
              <div className="topbar-right">
                ...
                <span className="pill pill-purple" style={{cursor:"pointer"}} onClick={()=>setAba("creditos")}>? {saldo} crÃ©ditos</span>
                {usuario?(
...
/ /   f o r c e   r e b u i l d   0 6 / 0 9 / 2 0 2 6   2 3 : 2 6 : 1 5  
 