...
async function callClaude(systemPrompt, userPrompt, historico=[]) {
  ...
  try {
    ...
    throw new Error("⚠️ Backend offline — inicie o servidor Python na porta 8082");
  }
...
const GRUPOS = [
  { label: "Criar", itens: [
    ...
    { id:"roteiro",       label:"Gerar Roteiro",      icon:"✍️" },
  ...
  { label: "Ferramentas", itens: [
    ...
    { id:"voz",           label:"Clonar Voz",         icon:"🎙️" },
...
const BOTTOM_NAV = [{ id:"chat",label:"Chat",icon:"✦" },{ id:"roteiro",label:"Roteiro",icon:"✍️" },{ id:"tendencias",label:"Trends",icon:"🔥" },{ id:"score_viral",label:"Score",icon:"⭐" },{ id:"analise",label:"Análise",icon:"📊" }];
const PAGE_META = {
...
const QC_FORMATOS = [
  ...
  { id:"roteiro-medio",label:"Roteiro 2-5min",emoji:"📱" },{ id:"legenda",label:"Legenda + Hashtags",emoji:"✍️" },
...
async function generateImage(prompt, modelo="pollinations", modeloObj=null) {
  ...
  if(modeloObj?.aiml || modelo.startsWith("aiml_")) {
    ...
    } catch { throw new Error("⚠️ Backend offline"); }
    if(!res.ok){const e=await res.json().catch(()=>({}));throw new Error(e?.detail||`HTTP ${res.status}`);}
  ...
  } catch { throw new Error("⚠️ Backend offline — inicie o servidor Python"); }
  if(!res.ok){const e=await res.json().catch(()=>({}));throw new Error(e?.detail||`HTTP ${res.status}`);}
  const d = await res.json();
...
// ==============================
const VOZES_PRONTAS = [
  { id:"pNInz6obpgDQGcFmaJgB", nome:"Adam",    desc:"Masculina grave, narração",  emoji:"🎙️" },
      ...
      <div style={{width:"100%",maxWidth:520,background:"var(--bg3)",border:".5px solid rgba(123,47,255,.3)",borderRadius:"var(--r-xl)",maxHeight:"90vh",overflow:"hidden",display:"flex",flexDirection:"column"}}>
        ...
        <div style={{padding:"1.25rem 1.5rem",borderBottom:".5px solid var(--border)",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
          <div style={{fontFamily:"var(--fh)",fontSize:16,fontWeight:800,color:"var(--text)"}}>
            {personagem ? "✏️ Editar Personagem" : "🎭 Criar Personagem"}
          </div>
        ...
        <div style={{display:"flex",gap:4,padding:"12px 16px 0",flexShrink:0}}>
          {[["aparencia","🎨 Aparência"],["voz","🎙️ Voz"],["padrao","⚙️ Padrões"]].map(([t,l])=>(
            ...
            <>
              ...
              <div style={{background:"rgba(123,47,255,.06)",border:".5px solid rgba(123,47,255,.2)",borderRadius:10,padding:"12px 14px"}}>
                ...
                <button className="btn btn-sm btn-ghost" onClick={()=>alert("Integre com ElevenLabs Voice Cloning API")}>
                  🎙️ Fazer upload de áudio para clonar
                </button>
              </div>
        ...
        <div key={p.id} className="card" style={{display:"flex",alignItems:"flex-start",gap:14,background:ativo?.id===p.id?"rgba(123,47,255,.06)":"var(--bg3)",border:ativo?.id===p.id?"1px solid rgba(123,47,255,.3)":".5px solid var(--border)"}}>
          ...
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:12,color:"var(--text3)",marginBottom:4}}>
              🎙️ {p.voice_nome} {p.voice_clonada?"(clonada)":""}
          ...
          <div style={{display:"flex",gap:6,flexShrink:0}}>
            ...
            <button className="btn btn-ghost btn-sm" onClick={()=>{setEditando(p);setShowModal(true);}}>✏️</button>
...
function VideoEditor({ onClose, onResult }) {
  ...
  const processar = async () => {
    ...
    try {
      ...
      setProgMsg("⚙️ Renderizando com Shotstack... (pode levar 2-3 min)");
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 4 }}>
          ...
          <a href={audioUrl} download="vortex-narracao.mp3"
            style={{ fontSize: 10, color: "var(--purple2)", textDecoration: "none",
              background: "rgba(123,47,255,.1)", border: ".5px solid rgba(123,47,255,.3)",
              borderRadius: 99, padding: "3px 9px" }}>⬇️</a>
  ...
  const enviar=useCallback(async(texto)=>{
    ...
    try{
      ...
      const routed = await vortexRouter(t, profile, memory, messages, historicoBackend);
        ...
        if (isRoteiro) {
          setMessages([...comUser, {role:"assistant", text:"✍️ Criando roteiro viral com Claude...", loading:true}]);
          try {
      ...
      if (routed.type === "image") {
        ...
        try {
          ...
          finalMsgs = [...comUser, {role:"assistant", text:`🖼️ **Imagem gerada!**

![imagem gerada](${url})

*Prompt usado: ${routed.prompt.slice(0,80)}...*`, imageUrl: url}];
        } catch(imgErr) {
          finalMsgs = [...comUser, {role:"assistant", text:`⚠️ Erro ao gerar imagem: ${imgErr.message}
...
[▶️ Clique para assistir](${url})

*Prompt: ${routed.prompt.slice(0,80)}...*`, videoUrl: url}];
        } catch(vidErr) {
          finalMsgs = [...comUser, {role:"assistant", text:`⚠️ Erro ao gerar vídeo: ${vidErr.message}`}];
        }
      } else {
      ...
      if (e.message?.includes("401") || e.message?.includes("API Key")) {
        ...
        msg = "⚠️ **Erro de conexão** — verifique sua internet. Se estiver com VPN, desative e tente novamente.";
      }
                      ...
                      {m.imageUrl && (
                        <div style={{marginTop:8}}>
                          ...
                          <div style={{display:"flex",gap:6,marginTop:6}}>
                            <a href={m.imageUrl} download="vortex-imagem.jpg" style={{fontSize:11,color:"var(--purple2)",textDecoration:"none",background:"rgba(123,47,255,.1)",border:".5px solid rgba(123,47,255,.3)",padding:"3px 10px",borderRadius:99}}>⬇️ Baixar</a>
                          </div>
                        </div>
                      )}
                      {m.videoUrl && (
                        <div style={{marginTop:8}}>
                          ...
                          <a href={m.videoUrl} download="vortex-video.mp4" style={{display:"inline-block",marginTop:6,fontSize:11,color:"var(--cyan)",textDecoration:"none",background:"rgba(34,211,238,.1)",border:".5px solid rgba(34,211,238,.3)",padding:"3px 10px",borderRadius:99}}>⬇️ Baixar vídeo</a>
                        </div>
                      )}
                      {m.audioUrl && (
                        <div style={{marginTop:8,background:"rgba(123,47,255,.08)",border:".5px solid rgba(123,47,255,.25)",borderRadius:12,padding:"10px 14px"}}>
                          <div style={{fontSize:11,color:"var(--purple2)",marginBottom:6,fontWeight:600}}>🎙️ Narração</div>
                          <audio src={m.audioUrl} controls style={{width:"100%",height:36}}/>
                          <a href={m.audioUrl} download="vortex-audio.mp3" style={{display:"inline-block",marginTop:6,fontSize:11,color:"var(--purple2)",textDecoration:"none",background:"rgba(123,47,255,.1)",border:".5px solid rgba(123,47,255,.3)",padding:"3px 10px",borderRadius:99}}>⬇️ Baixar áudio</a>
                        </div>
                      )}
                    </>
                  ) : (
...
function VideoFaceless() {
  ...
  const ETAPAS=["✍️ Gerando roteiro...","🎨 Criando imagens...","🎙️ Narrando...","📦 Finalizando..."];
    ...
    <div className="page">
      ...
      <div className="card" style={{marginBottom:"1rem",background:"linear-gradient(135deg,#0d0020,#1a003d)",border:"1px solid rgba(124,92,252,.3)"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6}}>
          {["✍️ Roteiro","🎨 Imagens","🎙️ Narração","📱 Pronto"].map((s,i)=>(
      ...
      <div className="card" style={{marginBottom:"1rem"}}>
        ...
        {erro&&<div style={{color:"#f87171",fontSize:13,marginBottom:8,padding:"8px 12px",background:"#f8717118",borderRadius:8}}>⚠️ {erro}</div>}
      ...
      {resultado&&(
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          ...
          {resultado.etapas?.audio&&(
            <div className="card">
              <div style={{fontWeight:600,fontSize:13,marginBottom:8}}>🎙️ Narração</div>
  ...
  return(
    <div className="page">
      ...
      <div className="card" style={{marginBottom:"1rem"}}>
        ...
        {erro&&<div style={{color:"#f87171",fontSize:13,marginBottom:8}}>⚠️ {erro}</div>}
      ...
      {resultado&&(
        <div className="card" style={{background:"linear-gradient(135deg,#0d0020,#1a003d)",border:"1px solid rgba(124,92,252,.3)"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:16}}>
            {[
              ["👁️","Views",(resultado.metricas?.views||0).toLocaleString("pt-BR")],
    ...
    <div className="page">
      ...
      <div className="card" style={{marginBottom:"1rem"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          {[
            {icon:"✍️",label:"Criar Roteiro",aba:"roteiro"},
      ...
      {!stats.perfil_configurado && (
        <div style={{background:"rgba(123,47,255,.08)",border:".5px solid rgba(123,47,255,.3)",borderRadius:"var(--r-lg)",padding:"1rem 1.25rem",textAlign:"center"}}>
          ...
          <button className="btn" onClick={()=>setAba("config")}>⚙️ Configurar agora</button>
        </div>
      )}
    </div>
  );
...
function AnalisePerfil({ setAba }) {
  ...
  const REDES = [
    ...
    {id:"youtube",   label:"YouTube",   icon:"▶️", cor:"#FF0000"},
          ...
          <div className="card">
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              <button className="btn" onClick={()=>setAba&&setAba("roteiro")}
                ...
                ✍️ Gerar roteiro para esse nicho
              </button>
      ...
      <div className="card" style={{marginBottom:12}}>
        ...
        <button className="btn btn-full" onClick={gerarRoteiro} disabled={loading||!tema.trim()}>
          {loading?<><div className="spinner"/>Gerando roteiro...</>:
           `✍️ Gerar Roteiro ${modo==="ab"?"A/B":modo==="serie"?"em Série":modo==="diretor"?"Diretor":""} — ${FORMATOS.find(f=>f.id===formato)?.desc||"2 créditos"}`}
        </button>
      </div>
          ...
          {scoreViral&&(
            <div className="card" style={{marginBottom:12,background:"rgba(123,47,255,.04)",border:`.5px solid ${scoreColor}44`,textAlign:"center"}}>
              ...
              <div style={{fontSize:13,fontWeight:600,color:scoreColor,marginTop:4}}>
                {scoreViral>=9?"🔥 Potencial viral altíssimo":scoreViral>=7?"⚡ Bom potencial viral":"⚠️ Pode melhorar"}
              </div>
          ...
          <div className="card" style={{marginBottom:12}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              ...
              <button className="btn" style={{fontSize:12,gridColumn:"1/-1"}} onClick={async()=>{
                ...
                🎙️ Narrar roteiro com IA (AIML TTS)
              </button>
            </div>
          </div>
...
const PLANOS_BASE = [
  ...
  { id:"pro", nome:"Pro", emoji:"🕳️", desc:"Alto volume",
    creditos:2500, imgs:250, vids:35, voz:true,
    promo:57, normal:99, cor:"#9d5cff", anim:"buraco", animLabel:"🕳️ Buraco Negro",
...
function GeradorImagens() {
  ...
  const MODELOS_IMG = [
    ...
    { id:"aiml_gpt_image",   nome:"🖼️ GPT Image 1.5",   desc:"AIML • OpenAI premium", creditos:10, cor:"#f472b6", endpoint:"aiml_gpt",     aiml:true, grupo:"AIML" },
      ...
      <div style={{display:"flex",flex:1,overflow:"auto",gap:0}}>
        ...
        <div style={{width:280,background:"#0a0015",borderLeft:"1px solid rgba(124,92,252,.2)",padding:"12px",overflow:"auto",flexShrink:0}}>
          ...
          <div style={{marginBottom:12}}>
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              {[["seta","👉"],["fogo","🔥"],["choque","😱"],["bomba","💥"],["alerta","⚠️"]].map(([id,emoji])=>(
...
function VortexStudio() {
  ...
  const ETAPAS = [
    { icon:"✍️", nome:"Criando roteiro",      desc:"Desenvolvendo narrativa e cenas..." },
    { icon:"🎙️", nome:"Gerando narração",     desc:"Sintetizando voz profissional..." },
      ...
      {!loading && !resultado && (
        <div className="card" style={{marginBottom:12}}>
          ...
          {erro&&<div style={{color:"#f87171",fontSize:12,marginBottom:8,
            padding:"8px 12px",background:"#f8717118",borderRadius:8}}>⚠️ {erro}</div>}
...
function GeradorVideos() {
  ...
  const MODELOS_VIDEO = [
    ...
    { id:"sora2",      nome:"🚀 Sora 2 Pro",       tag:"OpenAI",          creditos:120,cor:"#74AA9C", api:"fal", fal_model:"fal-ai/sora",                       plano_min:"elite",   desc:"OpenAI • API até set/2026", aviso:"⚠️ Consome 120 créditos por vídeo." },
    { id:"veo31_fast", nome:"🌟 Veo 3.1 Fast",     tag:"Google Premium",  creditos:160,cor:"#EF4444", api:"fal", fal_model:"fal-ai/veo3/fast",                  plano_min:"elite",   desc:"Google • 4K + áudio",       aviso:"⚠️ Consome 160 créditos por vídeo." },
    { id:"veo31",      nome:"⭐ Veo 3.1 Quality",  tag:"Melhor do mundo", creditos:325,cor:"#f72faa", api:"fal", fal_model:"fal-ai/veo3",                       plano_min:"elite",   desc:"Google • Máxima qualidade", aviso:"⚠️ Consome 325 créditos por vídeo." },
  ];
  ...
  const gerar = async () => {
    if(!prompt.trim()) return;
    setLoading(true); setErro(""); setResultado(null); setProgresso(0);
    const msgs = ["📤 Enviando prompt...","🎬 IA gerando vídeo...","⚙️ Renderizando...","📦 Finalizando..."];
            ...
            {modelo.creditos>0 && (
              <div style={{display:"flex",justifyContent:"space-between",padding:"8px 12px",
                ...
                <span style={{fontSize:11,color:modelo.creditos>50?"#fbbf24":"var(--text3)"}}>
                  {modelo.creditos>100?"⚠️ Alto consumo":modelo.creditos>30?"📊 Moderado":"✅ Econômico"}
                </span>
              </div>
            )}
...
function ClonadorVoz() {
  const VOZES=[
    ...
    {id:"pNInz6obpgDQGcFmaJgB",nome:"Adam",icon:"🎙️",desc:"Grave e autoritário — ideal para terror e suspense",estilo:"Grave"},
    ...
    <div className="page">
      ...
      <div className="card" style={{marginBottom:"1rem"}}>
        ...
        <button className="btn btn-full" onClick={gerar} disabled={loading||!texto.trim()}
          ...
          {loading?"⏳ Narrando...":"🎙️ Narrar com IA"}
        </button>
      </div>
...
function GeradorMusica() {
  const ESTILOS=[
    ...
    {id:"lofi_chill",label:"☁️ Lo-fi Chill",desc:"Relaxante — lifestyle",provider:"elevenlabs"},
...
function ScoreViral() {
  ...
  const extrairScores=(txt)=>{
    ...
    const dims=[
      ...
      {key:"retencao",label:"Retenção",icon:"👁️",color:"#22d3ee"},
      {key:"emocao",label:"Emoção",icon:"❤️",color:"#f87171"},
  ...
  const mediaLabel=sc?.media>=8?"🔥 Alto potencial viral":sc?.media>=6?"⚡ Potencial moderado":"⚠️ Precisa melhorar";
    ...
    <div className="page">
      ...
      <div className="card" style={{marginBottom:"1rem"}}>
        ...
        <button className="btn btn-full" onClick={analisar} disabled={loading||!roteiro.trim()}>
          {loading?<><div className="spinner"/>Analisando com IA...</>:"⭐ Analisar Score Viral — Grátis ∞"}
        </button>
      </div>
...
function MemoriaVortex() {
  ...
  const CAMPOS=[
    {key:"estilo_escrita",label:"Estilo de escrita",icon:"✍️",placeholder:"Ex: informal, com humor, direto ao ponto"},
...
function MeusProjetos() {
  ...
  const ICONS={chat:"💬",roteiro:"✍️",gerar_imagem:"🎨",gerar_video:"🎬",gerar_voz:"🎙️",gerar_musica:"🎵",score_viral:"⭐"};
        ...
        <div className="card" style={{marginBottom:12,background:"rgba(52,211,153,.04)",border:".5px solid rgba(52,211,153,.2)"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
            ...
            <div style={{display:"flex",gap:6,alignItems:"center"}}>
              ...
              <button className="btn" style={{fontSize:12,padding:"6px 14px"}} onClick={()=>setSel("starter")}>⬆️ Fazer upgrade</button>
            </div>
          </div>
        </div>
      )}
...
function Configuracoes() {
  ...
  return(
    <div className="page">
      ...
      {/* Abas */}
      <div style={{display:"flex",gap:6,marginBottom:16,background:"rgba(255,255,255,.04)",borderRadius:12,padding:4}}>
        {[{id:"perfil",icon:"👤",label:"Perfil"},{id:"preferencias",icon:"⚙️",label:"Preferências"},{id:"funcoes",icon:"🔧",label:"Funções"}].map(t=>(
      ...
      {aba==="preferencias"&&(
        <div className="card" style={{marginBottom:12}}>
          <div className="card-title" style={{marginBottom:14}}>⚙️ Preferências do app</div>
      ...
      {aba==="funcoes"&&(
        <div className="card" style={{marginBottom:12}}>
          ...
          {[
            ...
            {key:"roteiro",name:"Gerador de Roteiro",desc:"Roteiros virais com IA",icon:"✍️"},
            ...
            {key:"clonador_voz",name:"Clonar Voz",desc:"ElevenLabs voice cloning",icon:"🎙️"},
