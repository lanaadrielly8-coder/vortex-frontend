...
// Session ID �nico por dispositivo � identifica o usu�rio sem login
...
// Headers padr�o com Session-ID para todos os requests
...
const NICHOS_OB = ["Terror / True Crime","Gaming","Educacional","Humor","Lifestyle","Tecnologia","Fitness","Culin�ria","Finan�as","Beleza","M�sica","Esportes","Empreendedorismo","Viagem","Outro"];

const PLATS_OB = ["TikTok","Instagram","YouTube","YouTube Shorts","Twitter / X","Kwai","Pinterest"];

const DIAS_OB = ["Seg","Ter","Qua","Qui","Sex","S�b","Dom"];

const TONS_OB = ["Descontra�do","Educativo","Engra�ado","Motivacional","Direto","Storytelling","Pol�mico","�ntimo"];
...
const GRUPOS = [
  ...
  { label: "Criar", itens: [
    ...
    { id:"tendencias",    label:"Tend�ncias",         icon:"??" },

    { id:"imagens",       label:"Imagens IA",         icon:"??" },

    { id:"videos",        label:"V�deos IA",          icon:"??" },

  ]},

  { label: "Ferramentas", itens: [

    { id:"video_faceless",label:"V�deo Faceless",     icon:"??" },

    { id:"voz",           label:"Clonar Voz",         icon:"???" },

    { id:"musica",        label:"Gerar M�sica",       icon:"??" },

  ]},

  { label: "Analisar", itens: [

    { id:"analise",       label:"An�lise de Perfil",  icon:"??" },
...
const CONTA = [
  ...
  { id:"creditos",  label:"Cr�ditos",       icon:"??" },

  { id:"config",    label:"Configura��es",  icon:"??" },

];

const BOTTOM_NAV = [{ id:"chat",label:"Chat",icon:"?" },{ id:"roteiro",label:"Roteiro",icon:"??" },{ id:"tendencias",label:"Trends",icon:"??" },{ id:"score_viral",label:"Score",icon:"?" },{ id:"analise",label:"An�lise",icon:"??" }];

const PAGE_META = {

  chat:{title:"Vortex Chat",sub:"IA conversacional"},dashboard:{title:"Dashboard",sub:"Painel principal"},

  analise:{title:"An�lise de Perfil",sub:"Redes sociais"},

  analisar_video:{title:"An�lise de V�deo",sub:"Score e diagn�stico"},

  video_faceless:{title:"V�deo Faceless",sub:"1 clique � autom�tico"},roteiro:{title:"Gerar Roteiro",sub:"Roteiros & Hooks"},

  tendencias:{title:"Tend�ncias Virais",sub:"2026"},imagens:{title:"Gerador de Imagens",sub:"IA Generativa"},

  videos:{title:"Gerador de V�deos",sub:"WaveSpeed AI"},voz:{title:"Clonagem de Voz",sub:"ElevenLabs"},

  

  memoria:{title:"Mem�ria do Vortex",sub:"Prefer�ncias"},personagens:{title:"Meus Personagens",sub:"Biblioteca"},

  projetos:{title:"Meus Projetos",sub:"Hist�rico"},creditos:{title:"Comprar Cr�ditos",sub:"Planos"},

  config:{title:"Configura��es",sub:"Sistema"},
...
const QC_NICHOS = ["Terror","Gaming","Humor","Lifestyle","Educacional","Fitness","Tecnologia","Culin�ria"];
...
const QC_FORMATOS = [
  ...
  { id:"serie",label:"S�rie de 3 v�deos",emoji:"??" },
...
function QuickCreate({ onSend }) {
  ...
  const gerar=useCallback(async()=>{
    ...
    try {
      ...
      const map={
        ...
        hooks:`Crie 5 hooks virais para ${nicho}${tema?` sobre "${tema}"`:""}.Para cada: texto (m�x 7 palavras), emo��o ativada, por que funciona.`,
        ...
        legenda:`Crie legenda viral para ${nicho}${tema?` sobre "${tema}"`:""}.1� linha impactante, corpo, CTA, 15 hashtags.`,

        serie:`Crie s�rie 3 v�deos para ${nicho}${tema?` sobre "${tema}"`:""}.Cada um: t�tulo, hook, estrutura.`,

      };

      setResult(await callClaude("Voc� � o Vortex, especialista em conte�do viral. Responda em portugu�s com markdown.",map[fmt]));
  ...
  return (
    ...
    <>
      ...
      <button className="chat-action-btn" onClick={()=>setOpen(true)} title="Cria��o R�pida">
        ...
        <div style={mo.overlay} onClick={e=>e.target===e.currentTarget&&close()}>
          ...
          <div style={mo.modal}>
            ...
            <div style={mo.hd}>
              ...
              <span style={mo.hdT}>? Cria��o R�pida</span>
            ...
            {step===0&&<div style={mo.body}><span style={mo.lbl}>Qual � o seu nicho?</span><div style={mo.chips}>{QC_NICHOS.map(n=><button key={n} style={mo.chip(nicho===n)} onClick={()=>setNicho(n)}>{n}</button>)}</div><button style={mo.btn} onClick={()=>setStep(1)} disabled={!nicho}>Continuar ?</button></div>}
  ...
  return (
    ...
    <aside style={HS.panel}>
      ...
      <div style={HS.hd}>
        ...
        <span style={HS.lbl}>Hist�rico</span>
...
const INITIAL_MSGS = [{ role:"assistant", text:"Ol�! Sou o **Vortex**, sua IA para cria��o de conte�do viral. ??\n\nPosso te ajudar a criar roteiros, analisar perfis, encontrar tend�ncias e muito mais.\n\nComo posso te ajudar hoje?" }];

const SUGESTOES = ["Crie um roteiro viral para TikTok sobre terror","Quais s�o as tend�ncias de 2026?","Me d� 5 hooks poderosos para Reels","Analise meu nicho de gaming"];
...
function detectIntent(text) {
  ...
  // Detectar quando usu�rio quer texto, n�o gera��o

  const pedindoTexto = /prompt|me d[a�] o|escreve o|qual seria|me explica|como funciona|o que e|exemplo de|do roteiro|desse roteiro/.test(t);



  // -- IMAGEM � s� gera��o expl�cita ------------------------------
  ...
  // -- V�DEO + VOZ � combina os dois ------------------------------

  const temVideo = /(v�deo|video|clipe|clip|reels?|short|anima��o)/.test(t);
  ...
  const geraVideo = /^(gera|cria|faz|gerar|criar)\s+(um?\s+)?(v�deo|video|clipe|reel|short)/i.test(t) ||

                    /(gera|cria)\s+.*?(v�deo|video)/i.test(t);

  if(geraVideo && !pedindoTexto) return "video";



  // -- VOZ/�UDIO � narra��o expl�cita -----------------------------

  const geraVoz = /^(narra|narrar|l�|leia|fala|dubla)\s+/i.test(t) ||

                  /(narr(a|ar|a��o)|dubla(gem|r)|leitura em voz|gera (o )?�udio|tts|text.to.speech)/.test(t);
...
function extractTextToNarrate(userRequest, history) {
  ...
  // Se o usu�rio pediu pra narrar algo da conversa anterior

  const ref = /(narra|l�|leia|fala|dubla|voice).*(isso|esse|esse roteiro|esse texto|o roteiro|acima|anterior)/i;

  if (ref.test(userRequest)) {

    // Pega o �ltimo texto do assistente
  ...
  // Remove o comando e deixa s� o texto
...
// Detecta configura��es de voz do pedido

function extractVoiceConfig(userRequest) {

  const t = userRequest.toLowerCase();

  const config = { voice_id: "pNInz6obpgDQGcFmaJgB", stability: 0.5, similarity: 0.75 }; // padr�o Adam
  ...
  if (/(animado|energ�tico|hype|animada)/.test(t)) { config.stability = 0.3; config.similarity = 0.8; }
...
function buildChatSystem(profile, memory, history, modo) {
  ...
  const idioma   = profile.idioma || "Portugu�s";



  // Mem�ria do usu�rio � estilo, evitar, CTA, refer�ncias
  ...
  // -- MODO ASSISTENTE � IA geral, completa, sem restri��o de tema --

  if(modo === "assistente") {

    const lines = [

      "Voc� � o VORTEX AI � intelig�ncia artificial completa, equivalente ao Claude, Gemini e ChatGPT.",

      "",

      "CAPACIDADES TOTAIS:",

      "Responda qualquer pergunta de qualquer �rea: tecnologia, ci�ncia, hist�ria, filosofia, neg�cios,",

      "medicina, direito, matem�tica, programa��o, criatividade, produtividade, idiomas e muito mais.",

      "",

      "PERSONALIDADE:",

      "- Direto e inteligente � sem enrola��o, sem rob�tica",

      "- Honesto � se n�o souber, diz claramente em vez de inventar",

      "- Tom adapt�vel � t�cnico quando necess�rio, descontra�do quando o usu�rio quer",

      "- Contextual � mant�m coer�ncia com o que foi dito na conversa",

      "",

      "REGRAS CR�TICAS:",

      "- NUNCA invente dados, estat�sticas ou fatos",

      "- NUNCA diga que � Claude, Gemini, GPT � voc� � o VORTEX",

      "- Quando perguntarem sobre VOC� (suas capacidades) ? responda sobre VOC�",

      "- Responda SEMPRE em " + idioma,

    ];

    if(nome) lines.push("- Usu�rio: " + nome);

    if(nicho) lines.push("- Contexto do usu�rio: criador de conte�do de " + nicho);
  ...
  // -- MODO CRIADOR � especialista em conte�do viral --------------

  const lines = [

    "Voc� � o VORTEX AI � o assistente de cria��o de conte�do viral mais inteligente do Brasil.",

    "Voc� entende algoritmos, tend�ncias, psicologia do scroll e o mercado brasileiro profundamente.",
  ...
  if(nome || nicho || plats) {
    ...
    if(publicoAlvo) lines.push("- P�blico-alvo: " + publicoAlvo);
  ...
  const memoriaItems = [
    ...
    referencias     && "- Refer�ncias/inspira��es: " + referencias,
  ...
  if(memoriaItems.length > 0) {
    ...
    lines.push("MEM�RIA DO CRIADOR (use sempre):");
  ...
  lines.push(
    ...
    "- Tend�ncias em tempo real por nicho e plataforma",

    "- Estrat�gias de crescimento por algoritmo (TikTok, Reels, Shorts)",

    "- Copywriting que converte � gatilhos emocionais precisos",

    "- An�lise de m�tricas e diagn�stico de perfis",
    ...
    "- Responda em " + idioma + " natural � sem rob�tica",
...
async function vortexRouter(userText, profile, memory, history, historicoBackend=[]) {
  ...
  // Chat � monta hist�rico e system prompt sem template literals complexos
  ...
  // Envia s� a mensagem atual � hist�rico vai separado
...
const VOZES_PRONTAS = [
  ...
  { id:"pNInz6obpgDQGcFmaJgB", nome:"Adam",    desc:"Masculina grave, narra��o",  emoji:"???" },

  { id:"EXAVITQu4vr4xnSDxMaL", nome:"Sarah",   desc:"Feminina suave, conversa",   emoji:"??" },

  { id:"TX3LPaxmHKxFdv7VOQHJ", nome:"Liam",    desc:"Masculina jovem, energ�tico", emoji:"?" },
...
// Monta o prompt de imagem/v�deo com consist�ncia de personagem
...
// Detecta mudan�as pedidas pelo usu�rio em novo v�deo
...
function extractMudancas(userText) {
  ...
  if (t.includes("cenario") || t.includes("cen�rio")) {
  ...
  const exprs = ["sorrindo","s�rio","triste","animado","com raiva","surpreso","feliz"];
  ...
  if (t.includes("est�dio"))  mudancas.cenario = "studio, clean background";
...
// Modal de cria��o/edi��o de personagem
    ...
    <div style={{position:"fixed",inset:0,zIndex:500,background:"rgba(4,3,14,.85)",backdropFilter:"blur(8px)",display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      ...
      <div style={{width:"100%",maxWidth:520,background:"var(--bg3)",border:".5px solid rgba(123,47,255,.3)",borderRadius:"var(--r-xl)",maxHeight:"90vh",overflow:"hidden",display:"flex",flexDirection:"column"}}>
        ...
        <div style={{display:"flex",gap:4,padding:"12px 16px 0",flexShrink:0}}>
          ...
          {[["aparencia","?? Apar�ncia"],["voz","??? Voz"],["padrao","?? Padr�es"]].map(([t,l])=>(
          ...
          {tab==="aparencia" && (
            ...
            <>
              ...
              <div className="field">
                ...
                <label className="label">Descri��o visual (apar�ncia f�sica)</label>

                <textarea className="input" rows={4}

                  placeholder="Ex: homem jovem, 25 anos, cabelo preto curto, olhos castanhos, pele morena, express�o s�ria, estilo urbano..."

                  value={dados.aparencia} onChange={e=>set("aparencia",e.target.value)}/>

                <div style={{fontSize:10,color:"var(--text3)",marginTop:4}}>Quanto mais detalhado, mais consistente o personagem vai ficar entre os v�deos</div>
              ...
              <div className="field">
                ...
                <textarea className="input" rows={2}
                  ...
                  placeholder="Ex: engra�ado, direto, g�rias cariocas, sempre animado..."
          ...
          {tab==="voz" && (
            ...
            <>
              ...
              <div style={{background:"rgba(123,47,255,.06)",border:".5px solid rgba(123,47,255,.2)",borderRadius:10,padding:"12px 14px"}}>
                ...
                <div style={{fontSize:11,color:"var(--text3)",marginBottom:8}}>Grave 30 segundos da sua voz ou do personagem. O Vortex clona e usa sempre que gerar conte�do.</div>

                <button className="btn btn-sm btn-ghost" onClick={()=>alert("Integre com ElevenLabs Voice Cloning API")}>

                  ??? Fazer upload de �udio para clonar
              ...
              <div className="field" style={{marginTop:12}}>
                ...
                <label className="label">Tom de fala / instru��es</label>
          ...
          {tab==="padrao" && (
            ...
            <>
              ...
              <div className="field">
                ...
                <label className="label">Cen�rio padr�o</label>

                <input className="input" placeholder="Ex: est�dio moderno, fundo preto com neon roxo..."
              ...
              <div className="field">
                ...
                <label className="label">Roupa padr�o</label>

                <input className="input" placeholder="Ex: camiseta preta, bon�, corrente dourada..."
              ...
              <div className="field">
                ...
                <label className="label">Express�o padr�o</label>

                <div className="chips">

                  {["neutro","sorrindo","s�rio","animado","confiante"].map(ex=>(
              ...
              <div className="field">
                ...
                <label className="label">Pose padr�o</label>

                <input className="input" placeholder="Ex: de frente, olhando para c�mera, bra�os cruzados..."
        ...
        <div style={{padding:"1rem 1.5rem",borderTop:".5px solid var(--border)",display:"flex",gap:8,flexShrink:0}}>
          ...
          <button className="btn" style={{flex:2}} onClick={()=>onSave({...dados,id:dados.id||Date.now().toString(36)})}>
            ...
            ?? {personagem?"Salvar altera��es":"Criar personagem"}
...
// Componente da p�gina de personagens
  ...
  return (
    ...
    <div className="page">
      ...
      <div className="page-header">
        ...
        <p className="page-sub">Crie personagens consistentes para seus v�deos e conte�dos.</p>
      ...
      {ativo && (
        ...
        <div style={{background:"rgba(123,47,255,.08)",border:".5px solid rgba(123,47,255,.3)",borderRadius:"var(--r-lg)",padding:"12px 16px",marginBottom:"1.25rem",display:"flex",alignItems:"center",gap:12}}>
          ...
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:11,color:"var(--text3)"}}>Voz: {ativo.voice_nome} � Todos os v�deos usar�o este personagem</div>
...
const ESTILOS_EDICAO = [
  ...
  { id:"viral_tiktok",   nome:"?? Viral TikTok",    desc:"Legendas palavra a palavra, cortes r�pidos, zoom",     cor:"#f472b6" },

  { id:"cinematografico",nome:"?? Cinematogr�fico",  desc:"Cortes suaves, texto elegante, color grade escuro",    cor:"#a78bfa" },

  { id:"educacional",    nome:"?? Educacional",      desc:"Legendas limpas, destaques em palavras-chave",         cor:"#22d3ee" },

  { id:"terror",         nome:"?? Terror/Suspense",  desc:"Glitch, ru�do, texto tremendo, cortes abruptos",       cor:"#ef4444" },
...
// Transcri��o via AssemblyAI (ou fallback mock para dev)

async function transcribeVideo(videoUrl) {

  // Mock words � em produ��o o backend chama AssemblyAI
  ...
  // Fallback mock se backend n�o tiver endpoint de transcri��o ainda

  return [

    { text:"Ol� pessoal,",     start:0,    end:800  },

    { text:"hoje vou mostrar", start:900,  end:1800 },

    { text:"algo incr�vel",    start:1900, end:2700 },

    { text:"que vai mudar",    start:2800, end:3600 },

    { text:"seu conte�do!",    start:3700, end:4500 },
...
function VideoEditor({ onClose, onResult }) {
  ...
  const processar = async () => {
    ...
    try {
      ...
      setProgMsg("?? Enviando v�deo para o servidor...");
      ...
      if (!uploadRes.ok) throw new Error("Falha no upload do v�deo");

      const uploadData = await uploadRes.json();

      const videoUrl = uploadData.url; // URL p�blica retornada pelo backend



      setProgMsg("?? Preparando edi��o...");

      const words = []; // transcri��o opcional



      setProgMsg("?? Montando timeline de edi��o...");
    ...
    <div style={{position:"fixed",inset:0,zIndex:500,background:"rgba(4,3,14,.85)",backdropFilter:"blur(8px)",display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      ...
      <div style={{width:"100%",maxWidth:520,background:"var(--bg3)",border:".5px solid rgba(123,47,255,.3)",borderRadius:"var(--r-xl)",maxHeight:"90vh",overflow:"hidden",display:"flex",flexDirection:"column"}}>
        ...
        <div style={{padding:"1.25rem 1.5rem",borderBottom:".5px solid var(--border)",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
          ...
          <div style={{fontFamily:"var(--fh)",fontSize:16,fontWeight:800}}>?? Editar V�deo</div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"1.25rem 1.5rem"}}>
          ...
          {step===0 && (
            ...
            <>
              ...
              <div
                ...
                <div style={{fontFamily:"var(--fh)",fontSize:14,fontWeight:700,color:"var(--text)",marginBottom:6}}>Arraste o v�deo aqui</div>
          ...
          {step===1 && (
            ...
            <>
              ...
              <div className="field">
                ...
                <label className="label">Escolha o estilo de edi��o</label>
              ...
              <div style={{display:"flex",gap:8,marginTop:8}}>
                ...
                <button className="btn btn-ghost" onClick={()=>{setStep(0);setFile(null);setFileUrl("");}}>? Trocar v�deo</button>
          ...
          {step===2 && (
            ...
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:220,gap:16,textAlign:"center"}}>
              ...
              <div style={{fontSize:11,color:"var(--text3)"}}>Pode levar at� 3 minutos dependendo do v�deo</div>
...
function NarrarBtn({ texto }) {
  ...
  const narrar = async () => {
    ...
    try {
      ...
      if (!res || !res.ok) throw new Error("ElevenLabs indispon�vel");
...
function VortexChat() {
  ...
  const enviar=useCallback(async(texto)=>{
    ...
    try{
      ...
      // Carrega perfil e mem�ria do usu�rio
      ...
      // Prompt Engine � detecta inten��o e enriquece o prompt

      // Envia hist�rico formatado para o backend
...
Verifique sua VITE_WAVESPEED_API_KEY no .env`}];
        ...
        setMessages([...comUser, {role:"assistant", text:"?? Gerando v�deo com IA... (pode levar at� 2 min)", loading:true}]);

        try {

          const url = await generateVideo(routed.prompt);

          finalMsgs = [...comUser, {role:"assistant", text:`?? **V�deo gerado!**
...
*Prompt: ${routed.prompt.slice(0,80)}...*`, videoUrl: url}];
          ...
          finalMsgs = [...comUser, {role:"assistant", text:`?? Erro ao gerar v�deo: ${vidErr.message}`}];
      ...
      if (e.message?.includes("401") || e.message?.includes("API Key")) {
        ...
        // Retry silencioso � usu�rio n�o v� erro
        ...
        msg = "?? **Erro de conex�o** � verifique sua internet. Se estiver com VPN, desative e tente novamente.";
  ...
  const handleQuick=useCallback(texto=>{enviar(`Aqui est� o conte�do gerado:\n\n${texto}\n\nPode melhorar ou adaptar?`);},[enviar]);
    ...
    <div style={{display:"flex",height:"100%",width:"100%",overflow:"hidden",minHeight:0}}>
      ...
      <div style={{flex:1,minWidth:0,height:"100%",display:"flex",flexDirection:"column",overflow:"hidden",position:"relative"}}>
        ...
        <div className="chat-messages">
          ...
          {/* Empty state � aparece quando n�o tem mensagens */}
              ...
              <div>
                ...
                <div style={{fontSize:12,color:"var(--text3)",maxWidth:260,lineHeight:1.6}}>
                  ...
                  {window.__vortexModo==="assistente"
                    ...
                    :"Roteiros, estrat�gias, hooks, tend�ncias � tudo em um lugar."}

                </div>

              </div>

              {/* Sugest�es r�pidas no empty state */}
                    ...
                    <>
                      ...
                      {m.videoUrl && (
                        ...
                        <div style={{marginTop:8}}>
                          ...
                          <a href={m.videoUrl} download="vortex-video.mp4" style={{display:"inline-block",marginTop:6,fontSize:11,color:"var(--cyan)",textDecoration:"none",background:"rgba(34,211,238,.1)",border:".5px solid rgba(34,211,238,.3)",padding:"3px 10px",borderRadius:99}}>?? Baixar v�deo</a>
                      ...
                      {m.audioUrl && (
                        ...
                        <div style={{marginTop:8,background:"rgba(123,47,255,.08)",border:".5px solid rgba(123,47,255,.25)",borderRadius:12,padding:"10px 14px"}}>
                          ...
                          <div style={{fontSize:11,color:"var(--purple2)",marginBottom:6,fontWeight:600}}>??? Narra��o</div>

                          <audio src={m.audioUrl} controls style={{width:"100%",height:36}}/>

                          <a href={m.audioUrl} download="vortex-audio.mp3" style={{display:"inline-block",marginTop:6,fontSize:11,color:"var(--purple2)",textDecoration:"none",background:"rgba(123,47,255,.1)",border:".5px solid rgba(123,47,255,.3)",padding:"3px 10px",borderRadius:99}}>?? Baixar �udio</a>
          ...
          <div className="chat-input-outer">
            ...
            <div className="chat-input-box">
              ...
              <textarea ref={taRef} className="chat-textarea"
                ...
                placeholder={window.__vortexModo==="assistente"?"Pergunte qualquer coisa � como Claude, GPT e Gemini...":"Roteiro, estrat�gia, hook, tend�ncia � o que criar hoje?"}
            ...
            <div className="chat-input-actions">
              ...
              <button className="chat-action-btn" onClick={()=>setInput("Quais tend�ncias est�o bombando em ")}>

                <span style={{fontSize:16}}>??</span><span>Tend�ncia</span>

              </button>

              <button className="chat-action-btn" onClick={()=>setInput("Me d� ideias de conte�do para ")}>
...
function VideoFaceless() {
  ...
  const NICHOS=["terror","gaming","finan�as","fitness","lifestyle","tech","motivacional","curiosidades","crime real","anime"];
  ...
  return(
    ...
    <div className="page">
      ...
      <div className="page-header">
        ...
        <div className="eyebrow"><div className="eyebrow-dot"/>IA & Cria��o</div>

        <h1 className="page-title">V�deo Faceless <em>1 Clique</em></h1>

        <p className="page-sub">Digite o tema ? Vortex cria roteiro + imagens + narra��o autom�tico</p>
      ...
      <div className="card" style={{marginBottom:"1rem",background:"linear-gradient(135deg,#0d0020,#1a003d)",border:"1px solid rgba(124,92,252,.3)"}}>
        ...
        <div style={{fontSize:13,color:"rgba(200,180,255,.8)",marginBottom:12,lineHeight:1.6}}>
          ...
          ? <strong>Como funciona:</strong> Voc� digita o tema ? Vortex gera roteiro viral ? cria imagens para cada cena ? narra com voz profissional ? entrega tudo pronto

        </div>

        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6}}>

          {["?? Roteiro","?? Imagens","??? Narra��o","?? Pronto"].map((s,i)=>(
      ...
      <div className="card" style={{marginBottom:"1rem"}}>
        ...
        <div className="field">
          ...
          <textarea className="input" rows={3}
            ...
            placeholder="Ex: Um serial killer que ningu�m sabia que morava ao lado... / O segredo que a NASA esconde sobre Marte..."
        ...
        <button className="btn btn-full" onClick={gerar} disabled={loading||!tema.trim()}
          ...
          {loading?"? Criando seu v�deo...":"? Gerar V�deo Faceless"}
      ...
      {resultado&&(
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          ...
          <div className="card" style={{background:"#7C3AED18",border:"1px solid #7C3AED44"}}>
            ...
            <div style={{fontWeight:700,fontSize:14,color:"#A855F7",marginBottom:8}}>? V�deo pronto!</div>
          ...
          {resultado.etapas?.audio&&(
            ...
            <div className="card">
              ...
              <div style={{fontWeight:600,fontSize:13,marginBottom:8}}>??? Narra��o</div>
...
function AnalisarVideo() {
  ...
  const NICHOS=["viral","terror","gaming","finan�as","fitness","lifestyle","tech","humor","educa��o"];
  ...
  return(
    ...
    <div className="page">
      ...
      <div className="page-header">
        ...
        <div className="eyebrow"><div className="eyebrow-dot"/>An�lise</div>

        <h1 className="page-title">An�lise de <em>V�deo</em></h1>

        <p className="page-sub">Cole os dados do v�deo e descubra por que viralizou ou n�o.</p>
      ...
      <div className="card" style={{marginBottom:"1rem"}}>
        ...
        <div className="field">
          ...
          <label className="label">T�tulo do v�deo</label>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:12}}>
          ...
          <div className="field">
            ...
            <label className="label">?? Coment�rios</label>
        ...
        <button className="btn btn-full" onClick={analisar} disabled={loading||(!titulo&&!views)}
          ...
          {loading?"?? Analisando com IA...":"?? Analisar V�deo"}
  ...
  return (
    ...
    <div className="page">
      ...
      <div className="page-header">
        ...
        <h1 className="page-title">{nome ? "Ol�, "+nome.split(" ")[0]+" ??" : "Bem-vindo ao"} <em>{nome?"Vortex":"Vortex"}</em></h1>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:"1.25rem"}}>
        ...
        <div className="card" style={{textAlign:"center",padding:"1.25rem .75rem"}}>
          ...
          <div style={{fontSize:11,color:"var(--text3)",marginTop:4}}>?? Cr�ditos</div>
        ...
        <div className="card" style={{textAlign:"center",padding:"1.25rem .75rem",cursor:"pointer"}} onClick={()=>setAba("videos")}>
          ...
          <div style={{fontSize:11,color:"var(--text3)",marginTop:4}}>Gerar V�deo</div>
      ...
      <div className="card" style={{marginBottom:"1rem",background:"rgba(123,47,255,.04)",border:"0.5px solid rgba(123,47,255,.2)"}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
          ...
          <div className="card-title">?? C�rebro do Vortex</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:10}}>
          ...
          <div style={{textAlign:"center",padding:8,background:"rgba(255,255,255,.03)",borderRadius:8}}>
            ...
            <div style={{fontSize:10,color:"var(--text3)"}}>taxa aprova��o</div>
      ...
      <div className="card" style={{marginBottom:"1rem"}}>
        ...
        <div className="card-title" style={{marginBottom:".75rem"}}>? A��es R�pidas</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          ...
          {[
            ...
            {icon:"??",label:"Ver Tend�ncias",aba:"tendencias"},
  ...
  return (
    ...
    <div className="page">
      ...
      <div className="page-header">
        ...
        <h1 className="page-title">An�lise de <em>Perfil</em></h1>

        <p className="page-sub">Dados reais + recomenda��es IA para crescer.</p>
      ...
      <div className="card" style={{marginBottom:"1rem"}}>
        ...
        <button className="btn btn-full" onClick={analisar} disabled={loading||!perfil.trim()}>
          ...
          {loading ? "?? Analisando..." : `?? Analisar ${redeAtual?.label} � 2 cr�ditos`}
      ...
      {dados && (
        ...
        <>
          ...
          {(dados.analise_completa || dados.recomendacao_ia) && (
            ...
            <div className="card" style={{marginBottom:"1rem",border:".5px solid rgba(123,47,255,.2)"}}>
              ...
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
                ...
                <div className="card-title">?? An�lise do Vortex AI</div>
          ...
          <div className="card">
            ...
            <div className="card-title" style={{marginBottom:10}}>? A��es r�pidas</div>
...
function Roteiro() {
  ...
  const MODOS = [
    ...
    {id:"diretor",   icon:"??", label:"Diretor",   desc:"Frame a frame com dire��o de c�mera"},

    {id:"serie",     icon:"??", label:"S�rie",     desc:"3 epis�dios com cliffhanger"},

    {id:"ab",        icon:"??", label:"A/B",       desc:"2 vers�es para testar qual performa mais"},

    {id:"faceless",  icon:"??", label:"Faceless",  desc:"Sem mostrar rosto � narra��o + imagens"},

    {id:"anuncio",   icon:"??", label:"An�ncio",   desc:"Roteiro de convers�o para vender"},

  ];

  const FORMATOS = [

    {id:"curto",    label:"30s", desc:"1 cr�dito"},

    {id:"medio",    label:"60s", desc:"2 cr�ditos"},

    {id:"longo",    label:"3min",desc:"3 cr�ditos"},

    {id:"completo", label:"5min",desc:"5 cr�ditos"},

  ];

  const NICHOS = ["terror","gaming","finan�as","fitness","lifestyle","tech","motivacional","culin�ria","humor","anime","educacional","empreendedorismo"];
  ...
  const extrairScore = (txt) => {
    ...
    const m = txt.match(/M�DIA[:\s]+(\d+(?:[.,]\d+)?)/i) ||

              txt.match(/m�dia[:\s]+(\d+(?:[.,]\d+)?)/i) ||
  ...
  const gerarRoteiro = async () => {
    ...
    const etapas = [
      ...
      {p:8,  msg:"Pesquisando tend�ncias reais..."},
    ...
    <div className="page">
      ...
      <div className="card" style={{marginBottom:12}}>
        ...
        <div className="field">
          ...
          <textarea className="input" rows={3}
            ...
            placeholder={
              ...
              modo==="terror"?"Ex: O apartamento que ningu�m conseguia alugar por mais de 30 dias...":

              modo==="anuncio"?"Ex: App que dobrou minha renda em 30 dias...":

              "Ex: O experimento psicol�gico que provou que todos mentem..."
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          ...
          <div className="field">
            ...
            <label className="label">Dura��o</label>
          ...
          <div style={{marginBottom:12}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:"var(--text3)",marginBottom:6}}>
              ...
              <span>
                ...
                {progresso<20?"?? Pesquisando tend�ncias...":
        ...
        <button className="btn btn-full" onClick={gerarRoteiro} disabled={loading||!tema.trim()}>
          ...
          {loading?<><div className="spinner"/>Gerando roteiro...</>:
           ...
           `?? Gerar Roteiro ${modo==="ab"?"A/B":modo==="serie"?"em S�rie":modo==="diretor"?"Diretor":""} � ${FORMATOS.find(f=>f.id===formato)?.desc||"2 cr�ditos"}`}
      ...
      {resultado&&(
        ...
        <div>
          ...
          {scoreViral&&(
            ...
            <div className="card" style={{marginBottom:12,background:"rgba(123,47,255,.04)",border:`.5px solid ${scoreColor}44`,textAlign:"center"}}>
              ...
              <div style={{fontSize:13,fontWeight:600,color:scoreColor,marginTop:4}}>
                ...
                {scoreViral>=9?"?? Potencial viral alt�ssimo":scoreViral>=7?"? Bom potencial viral":"?? Pode melhorar"}
          ...
          <div className="card" style={{marginBottom:12}}>
            ...
            <div className="card-title" style={{marginBottom:10}}>?? Pr�ximos passos</div>

            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>

              <button className="btn" style={{fontSize:12}} onClick={()=>{setTema(tema+" � vers�o 2");gerarRoteiro();}}>
              ...
              <button className="btn" style={{fontSize:12,gridColumn:"1/-1"}} onClick={async()=>{
                ...
                try{
                  ...
                  const a=document.createElement("a");a.href=audioUrl;a.download="narra��o-vortex.mp3";a.click();

                  alert("? Narra��o gerada e baixando!");
          ...
          {feedbackDado&&<div style={{textAlign:"center",color:"#34d399",fontSize:13,padding:8}}>? Vortex aprendeu! Pr�ximos roteiros ser�o melhores.</div>}
      ...
      {historico.length>0&&!resultado&&(
        ...
        <div className="card">
          ...
          <div className="card-title" style={{marginBottom:10}}>?? Hist�rico desta sess�o</div>
...
function Tendencias() {
  ...
  // Google Trends � carrega autom�tico
    ...
    try{
      ...
      if(aba==="nicho"){
        ...
        const r=await callClaude(
          ...
          "Especialista em tend�ncias virais de redes sociais. Responda em portugu�s.",

          `Liste as top 8 tend�ncias de conte�do para ${nicho} no ${plataforma} em 2026. Para cada: nome, por que viraliza, dica pr�tica de uso. Responda SOMENTE JSON: {"tendencias":[{"nome":"...","motivo":"...","dica":"...","emoji":"..."}]}`
  ...
  const NICHOS=["terror","gaming","finan�as","fitness","lifestyle","tech","motivacional","culin�ria","humor","anime"];
  ...
  return(
    ...
    <div className="page">
      ...
      <div className="page-header">
        ...
        <h1 className="page-title">Tend�ncias <em>Virais</em></h1>

        <p className="page-sub">Google Trends em tempo real + an�lise por nicho.</p>
        ...
        <div style={{display:"inline-flex",alignItems:"center",gap:6,marginTop:6,
          ...
          <span style={{fontSize:11,color:"#34d399",fontWeight:600}}>Gr�tis e ilimitado � dispon�vel em todos os planos</span>
      ...
      <div style={{display:"flex",gap:6,marginBottom:16,background:"rgba(255,255,255,.04)",borderRadius:12,padding:4}}>
        ...
        {[
          ...
          {id:"global",icon:"??",label:"Tend�ncia Global"},
        ...
        <div>
          ...
          <div className="card" style={{marginBottom:12}}>
            ...
            <button className="btn btn-full" onClick={buscar} disabled={loading}>
              ...
              {loading?<><div className="spinner"/>Analisando...</>:"?? Ver top 8 tend�ncias"}
        ...
        <div>
          ...
          <div className="card" style={{marginBottom:12}}>
            ...
            <button className="btn btn-full" onClick={buscar} disabled={loading}>
              ...
              {loading?<><div className="spinner"/>Buscando...</>:"?? Ver tend�ncia global"}
          ...
          {dados?.lista&&(
            ...
            <div className="card">
              ...
              <div className="card-title" style={{marginBottom:12}}>?? Tend�ncias Globais</div>
...
const PLANO_FREE = {
  ...
  features:[
    ...
    "?? Tend�ncias em tempo real � ILIMITADO",
...
const PLANOS_BASE = [
  ...
  { id:"starter", nome:"Starter", emoji:"?", desc:"Para come�ar",
    ...
    features:["250 cr�ditos/m�s","25 imagens IA","4 v�deos","Roteiros ilimitados","Chat com Gemini"],
  ...
  { id:"creator", nome:"Creator", emoji:"??", desc:"Postar todo dia",
    ...
    promo:27, normal:49, cor:"#22d3ee", anim:"galaxia", animLabel:"?? Gal�xia", popular:true,

    preco_mensal:27, preco_anual:22, creditos_anual:1100,

    features:["900 cr�ditos/m�s","90 imagens IA","12 v�deos","Clonagem de voz","Chat com GPT-4o","Score Viral","Tend�ncias em tempo real"],
  ...
  { id:"pro", nome:"Pro", emoji:"???", desc:"Alto volume",
    ...
    features:["2.500 cr�ditos/m�s","250 imagens IA","35 v�deos","Clonagem de voz","Chat com Claude Haiku","An�lise de perfil avan�ada","Mem�ria do Vortex","Suporte priorit�rio"],

    modelo:"Claude Haiku"

  },

  { id:"elite", nome:"Elite", emoji:"?", desc:"Sem preocupa��o",

    creditos:8000, imgs:800, vids:120, voz:true,

    promo:97, normal:199, cor:"#f472b6", anim:"portal", animLabel:"? Portal C�smico",

    preco_mensal:97, preco_anual:80, creditos_anual:10000,

    features:["8.000 cr�ditos/m�s","800 imagens IA","120 v�deos","Clonagem de voz premium","Chat com Claude Sonnet","Todos os recursos","API pr�pria (em breve)","Suporte VIP 24h"],
...
const PACKS = [

  {
    ...
    id:"basico", nome:"Pack B�sico", emoji:"?", creditos:500,

    promo:15, normal:19, cor:"#fbbf24",

    desc:"Ideal para testar mais � sem prazo de validade",

    features:["500 cr�ditos extras","Sem expira��o","~50 roteiros ou 100 imagens"],
...

  {
    ...
    desc:"Melhor custo-benef�cio � 22% mais barato por cr�dito",

    features:["2.000 cr�ditos extras","Sem expira��o","~200 roteiros ou 400 imagens"],
...

  {
    ...
    desc:"Para quem cria muito � 40% mais barato por cr�dito",

    features:["5.000 cr�ditos extras","Sem expira��o","~500 roteiros ou 1.000 imagens"],
...
function GeradorImagens() {
  ...
  const ESTILOS = [
    ...
    { id:"cinematico", label:"?? Cinematogr�fico", prompt:"cinematic, movie scene, dramatic lighting" },
  ...
  const MODELOS_IMG = [
    ...
    { id:"pollinations",   nome:"?? Pollinations",      desc:"Ilimitado � Gr�tis",     creditos:0, cor:"#10B981", endpoint:"pollinations", free:true, grupo:"Gr�tis" },

    { id:"hf_flux",        nome:"?? FLUX Schnell HF",   desc:"HuggingFace � Gr�tis",   creditos:0, cor:"#F59E0B", endpoint:"hf_flux",      free:true, grupo:"Gr�tis" },

    { id:"gemini",         nome:"?? Nano Banana Pro",   desc:"Google � Gr�tis",        creditos:0, cor:"#4285F4", endpoint:"gemini",       free:true, grupo:"Gr�tis" },

    // AIML � usa cr�ditos da sua key

    { id:"aiml_flux_schnell",nome:"? FLUX Schnell AIML", desc:"AIML � R�pido",        creditos:5, cor:"#7b2fff", endpoint:"aiml_flux",    aiml:true, grupo:"AIML" },
    ...
    // FAL premium � cr�ditos calculados com margem de seguran�a real
      ...
      <div style={{marginBottom:"1rem"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:6}}>
          ...
          {[
            ...
            {img:"https://picsum.photos/seed/terror1/300/200",tag:"Terror",tagCor:"#ef4444",titulo:"A Noite Que Mudou Tudo",sub:"True crime � 2.3M views",prompt:"homem sozinho em corredor escuro, sombra amea�adora, terror psicol�gico, cinematogr�fico, thumbnail viral"},

            {img:"https://picsum.photos/seed/gaming2/300/200",tag:"Gaming",tagCor:"#3b82f6",titulo:"Level Imposs�vel",sub:"FPS gameplay � 4.1M views",prompt:"personagem de game em batalha �pica, explos�es neon, thumbnail viral gaming TikTok"},

            {img:"https://picsum.photos/seed/crypto3/300/200",tag:"Finan�as",tagCor:"#f59e0b",titulo:"R$50k em 30 Dias",sub:"Crypto � 1.8M views",prompt:"homem surpreso com gr�fico de crypto explodindo, dinheiro, thumbnail viral finan�as"},

            {img:"https://picsum.photos/seed/sunrise4/300/200",tag:"Lifestyle",tagCor:"#10b981",titulo:"Acordei �s 4h por 30d",sub:"Motivacional � 3.5M views",prompt:"silhueta dram�tica ao nascer do sol, c�u laranja e roxo, motivacional, thumbnail viral"},

            {img:"https://picsum.photos/seed/anime5/300/200",tag:"Anime",tagCor:"#ec4899",titulo:"Protagonista �pico",sub:"Edit � 5.2M views",prompt:"personagem anime em pose �pica com aura de poder, estilo Jujutsu Kaisen, thumbnail viral"},

            {img:"https://picsum.photos/seed/dark6/300/200",tag:"Dark Art",tagCor:"#6b7280",titulo:"Segredo Sombrio",sub:"Mist�rio � 2.8M views",prompt:"rosto misterioso emergindo das sombras, luz dram�tica, atmosfera dark cinematogr�fica"},

            {img:"https://picsum.photos/seed/tech7/300/200",tag:"Tech",tagCor:"#06b6d4",titulo:"IA vai te substituir",sub:"Tecnologia � 6.1M views",prompt:"rob� futurista com IA, c�digo hologr�fico, est�tica cyberpunk, thumbnail viral tech"},

            {img:"https://picsum.photos/seed/fitness8/300/200",tag:"Fitness",tagCor:"#f97316",titulo:"Transforma��o 90 dias",sub:"Fitness � 4.4M views",prompt:"transforma��o f�sica dram�tica, luz intensa de academia, silhueta musculosa, thumbnail fitness"},
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(130px,1fr))",gap:8}}>
          ...
          {MODELOS_IMG.map(m=>(
            ...
            <button key={m.id} onClick={()=>setModelo(m)}
              ...
              <span style={{fontSize:11,color:"var(--text3)",fontWeight:600}}>?? {m.creditos} cr�ditos</span>
      ...
      <div className="card" style={{marginBottom:"1rem"}}>
        ...
        <div className="field">
          ...
          <textarea className="input" rows={4}
            ...
            placeholder="Ex: homem olhando para explos�o de luz, perspectiva dram�tica, cores vibrantes, thumbnail TikTok..."
        ...
        <div style={{background:"#7C3AED12",border:"1px solid #7C3AED30",borderRadius:8,padding:"8px 12px",marginBottom:12,display:"flex",justifyContent:"space-between"}}>
          ...
          <span style={{fontSize:13,color:"var(--text2)"}}>?? {modelo.creditos} cr�ditos</span>
        ...
        {modelo.creditos>0&&(
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
            ...
            <span style={{fontSize:12,color:"var(--text3)"}}>?? Custo: <strong style={{color:"var(--text2)"}}>{modelo.creditos} cr�ditos</strong></span>
        ...
        <div className="card">
          ...
          <div style={{display:"flex",gap:8,justifyContent:"center",marginTop:8}}>
            ...
            <button onClick={()=>{
              ...
              alert("? Vortex aprendeu! Pr�ximas imagens ser�o ainda melhores.");
      ...
      <div style={{display:"flex",flex:1,overflow:"auto",gap:0}}>
        ...
        <div style={{width:280,background:"#0a0015",borderLeft:"1px solid rgba(124,92,252,.2)",padding:"12px",overflow:"auto",flexShrink:0}}>
          ...
          <div style={{marginBottom:12}}>
            ...
            <label style={{fontSize:11,color:"var(--text3)",display:"block",marginBottom:4}}>T�TULO</label>
          ...
          <div style={{marginBottom:12}}>
            ...
            <label style={{fontSize:11,color:"var(--text3)",display:"block",marginBottom:4}}>SUBT�TULO</label>
...
// VORTEX STUDIO � Pipeline completo de v�deo IA

// Claude ? ElevenLabs ? FLUX ? Kling = V�deo �nico Vortex
...
function VortexStudio() {
  ...
  const ESTILOS = [
    ...
    { id:"cinematografico", nome:"?? Cinematogr�fico", desc:"Dark e dram�tico" },

    { id:"viral",           nome:"?? Viral",           desc:"Vibrante e din�mico" },
  ...
  const MOTORES = [
    ...
    { id:"wan22_fast",  nome:"? R�pido",      creditos:60,  plano:"starter", desc:"Gera��o em segundos" },

    { id:"kling3_std",  nome:"?? Padr�o",      creditos:90,  plano:"creator", desc:"Alta qualidade 4K" },
    ...
    { id:"veo31_fast",  nome:"?? Ultra",       creditos:185, plano:"elite",   desc:"M�xima qualidade poss�vel" },
  ...
  const ETAPAS = [
    ...
    { icon:"???", nome:"Gerando narra��o",     desc:"Sintetizando voz profissional..." },

    { icon:"??", nome:"Renderizando cena",    desc:"Construindo identidade visual..." },

    { icon:"??", nome:"Animando v�deo",       desc:"Dando vida � cena final..." },
  ...
  async function criar() {
    ...
    if(!tema.trim()) { setErro("Descreva o tema do seu v�deo"); return; }
    ...
    try {
      ...
      setErro(e.message || "Erro ao criar v�deo. Tente novamente.");
  ...
  return (
    ...
    <div className="page">
      ...
      <div className="page-header">
        ...
        <p className="page-sub">Descreva sua ideia. O Vortex cria o v�deo completo � roteiro, voz, imagem e movimento.</p>
      ...
      {!loading && !resultado && (
        ...
        <div className="card" style={{marginBottom:12}}>
          ...
          <label className="label" style={{fontSize:13}}>Qual � a ideia do seu v�deo?</label>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
            ...
            <div>
              ...
              <select className="input" value={nicho} onChange={e=>setNicho(e.target.value)}>
                ...
                {["geral","terror","gaming","finan�as","fitness","tech","lifestyle","anime","motivacional"].map(n=>(
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",
            ...
            <span style={{fontSize:12,color:"var(--text3)"}}>
              ...
              ?? <strong style={{color:"var(--text)"}}>{motorAtual.creditos} cr�ditos</strong> � {motorAtual.desc}
          ...
          <button className="btn btn-full" onClick={criar} disabled={!tema.trim()}
            ...
            ?? Criar V�deo � {motorAtual.creditos} cr�ditos
      ...
      {loading && (
        ...
        <div className="card" style={{textAlign:"center",padding:"2rem 1rem"}}>
          ...
          <div style={{fontFamily:"var(--fh)",fontSize:20,fontWeight:800,
            ...
            Vortex Studio est� criando...

          </div>

          <div style={{fontSize:13,color:"var(--text3)",marginBottom:24}}>

            Isso pode levar at� 1 minuto. N�o feche essa tela.
...
function GeradorVideos() {
  ...
  const MODELOS_VIDEO = [
    ...
    { id:"studio",     nome:"? Vortex Studio",  tag:"Exclusivo",       creditos:90, cor:"#9333ea", api:"studio", plano_min:"creator", desc:"Pipeline completo � roteiro, voz, imagem e v�deo", aviso:null, studio:true },

    { id:"hf_ltx",     nome:"?? LTX-Video",      tag:"Gr�tis",          creditos:0,  cor:"#F59E0B", api:"hf",  free:true,   plano_min:"free",    desc:"HuggingFace � Open source",         aviso:null },

    { id:"wan26_fast", nome:"? WAN 2.6 Fast",    tag:"Open source",     creditos:5,  cor:"#7C3AED", api:"fal", fal_model:"fal-ai/wan/t2v-1.3b",               plano_min:"starter", desc:"Alibaba � R�pido",           aviso:null },

    { id:"hailuo23",   nome:"?? Hailuo 2.3",      tag:"Custo-benef�cio", creditos:10, cor:"#8B5CF6", api:"fal", fal_model:"fal-ai/minimax/video-01-live",       plano_min:"starter", desc:"MiniMax � menos de 30s",    aviso:null },
    ...
    { id:"seedance2",  nome:"?? Seedance 2.0",     tag:"#1 sem �udio",   creditos:15, cor:"#F59E0B", api:"fal", fal_model:"fal-ai/bytedance/seedance-1-lite",  plano_min:"creator", desc:"ByteDance � Top movement",  aviso:null },
    ...
    { id:"veo31_lite", nome:"? Veo 3.1 Lite",     tag:"Google",          creditos:15, cor:"#9d5cff", api:"fal", fal_model:"fal-ai/veo3/lite",                  plano_min:"pro",     desc:"Google � �udio nativo",     aviso:null },

    { id:"sora2",      nome:"?? Sora 2 Pro",       tag:"OpenAI",          creditos:120,cor:"#74AA9C", api:"fal", fal_model:"fal-ai/sora",                       plano_min:"elite",   desc:"OpenAI � API at� set/2026", aviso:"?? Consome 120 cr�ditos por v�deo." },

    { id:"veo31_fast", nome:"?? Veo 3.1 Fast",     tag:"Google Premium",  creditos:160,cor:"#EF4444", api:"fal", fal_model:"fal-ai/veo3/fast",                  plano_min:"elite",   desc:"Google � 4K + �udio",       aviso:"?? Consome 160 cr�ditos por v�deo." },

    { id:"veo31",      nome:"? Veo 3.1 Quality",  tag:"Melhor do mundo", creditos:325,cor:"#f72faa", api:"fal", fal_model:"fal-ai/veo3",                       plano_min:"elite",   desc:"Google � M�xima qualidade", aviso:"?? Consome 325 cr�ditos por v�deo." },
  ...
  const gerar = async () => {
    ...
    const msgs = ["?? Enviando prompt...","?? IA gerando v�deo...","?? Renderizando...","?? Finalizando..."];
    ...
    try {
      ...
      setErro(e.message || "Erro ao gerar v�deo");
  ...
  return (
    ...
    <div className="page">
      ...
      <div className="page-header">
        ...
        <h1 className="page-title">Gerador de <em>V�deo</em></h1>
      ...
      <div>
          ...
          <div className="card" style={{marginBottom:12}}>
            ...
            <textarea className="input" rows={3} value={prompt}
              ...
              placeholder="Ex: Uma floresta densa � noite, neblina surgindo entre as �rvores..."
            ...
            {modelo.creditos>0 && (
              ...
              <div style={{display:"flex",justifyContent:"space-between",padding:"8px 12px",
                ...
                <span style={{fontSize:12,color:"var(--text3)"}}>
                  ...
                  {"?? Custo: "}<strong style={{color:"var(--text)"}}>{modelo.creditos+" cr�ditos"}</strong>

                </span>

                <span style={{fontSize:11,color:modelo.creditos>50?"#fbbf24":"var(--text3)"}}>

                  {modelo.creditos>100?"?? Alto consumo":modelo.creditos>30?"?? Moderado":"? Econ�mico"}
            ...
            {!loading && (
              ...
              <button className="btn btn-full" onClick={gerar} disabled={!prompt.trim()}
                ...
                {"?? Gerar V�deo � "+modelo.creditos+" cr�ditos"}
          ...
          {resultado && (
            ...
            <div className="card">
              ...
              <div style={{fontWeight:700,marginBottom:12,color:"#7C3AED"}}>{"? V�deo gerado!"}</div>
...
function ClonadorVoz() {
  ...
  const VOZES=[
    ...
    {id:"onwK4e9ZLuTAKqWW03F9",nome:"VORTEX",icon:"??",desc:"Voz oficial do Vortex � dram�tica e envolvente",estilo:"�pico"},

    {id:"pNInz6obpgDQGcFmaJgB",nome:"Adam",icon:"???",desc:"Grave e autorit�rio � ideal para terror e suspense",estilo:"Grave"},

    {id:"TX3LPaxmHKxFdv7VOQHJ",nome:"Liam",icon:"?",desc:"Jovem e energ�tico � perfeito para gaming",estilo:"Energ�tico"},

    {id:"ErXwobaYiN019PkySvjV",nome:"Antoni",icon:"??",desc:"Tom suave e narrativo � �timo para storytelling",estilo:"Narrativo"},

    {id:"VR6AewLTigWG4xSOukaG",nome:"Arnold",icon:"??",desc:"Imponente e confiante � autoridade m�xima",estilo:"Imponente"},

    {id:"EXAVITQu4vr4xnSDxMaL",nome:"Bella",icon:"??",desc:"Feminina e carism�tica � engajamento alto",estilo:"Carism�tico"},
  ...
  return(
    ...
    <div className="page">
      ...
      <div className="page-header">
        ...
        <h1 className="page-title">Narra��o com IA</h1>

        <p className="page-sub">Transforme seu roteiro em narra��o profissional</p>

      </div>



      {/* Player de demo de narra��o */}
      ...
      <div style={{marginBottom:"1rem"}}>
        ...
        <div style={{fontSize:12,color:"var(--text3)",marginBottom:8,display:"flex",alignItems:"center",gap:6}}>
          ...
          Estilos de narra��o � clique para usar o texto
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
          ...
          {[
            ...
            {icon:"??",cor:"#ef4444",bg:"linear-gradient(135deg,#1a0000,#3d0000)",titulo:"Terror & Suspense",sub:"Voz grave e dram�tica",exemplo:"Ningu�m sabia o que estava prestes a acontecer naquela noite. O sil�ncio era ensurdecedor..."},

            {icon:"?",cor:"#3b82f6",bg:"linear-gradient(135deg,#00051a,#001040)",titulo:"Gaming Hype",sub:"Energ�tico e r�pido",exemplo:"Esse glitch quebrou o jogo INTEIRO! Ningu�m consegue passar desse n�vel... at� agora!"},

            {icon:"??",cor:"#f59e0b",bg:"linear-gradient(135deg,#1a0d00,#3d1f00)",titulo:"Finan�as",sub:"Autorit�rio e confiante",exemplo:"Esse ativo vai triplicar em 2025. Aqui est� o motivo que ningu�m est� te contando..."},

            {icon:"??",cor:"#a855f7",bg:"linear-gradient(135deg,#0d0020,#2d0057)",titulo:"Storytelling",sub:"Narrativo e envolvente",exemplo:"Era uma noite fria quando tudo mudou para sempre. Eu n�o sabia que aquela seria minha �ltima chance..."},
      ...
      <div className="card" style={{marginBottom:"1rem"}}>
        ...
        <div style={{background:"#7C3AED12",border:"1px solid #7C3AED30",borderRadius:8,padding:"8px 12px",marginBottom:12,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <span style={{fontSize:13,color:"var(--text2)"}}>?? {Math.ceil(texto.length/100)} cr�ditos</span>
      ...
      {resultado&&(
        ...
        <div className="card">
          ...
          <div style={{fontWeight:700,marginBottom:16,color:"#7C3AED"}}>? Narra��o pronta!</div>
...
function GeradorMusica() {
  ...
  const ESTILOS=[
    ...
    {id:"dark_electronic",label:"?? Dark Electronic",desc:"Tens�o � terror/thriller",provider:"elevenlabs"},

    {id:"cinematic",label:"?? Cinematogr�fico",desc:"�pico � trailers",provider:"elevenlabs"},
    ...
    {id:"ambient",label:"?? Ambient",desc:"Atmosf�rico � ASMR",provider:"elevenlabs"},

    {id:"horror",label:"?? Horror",desc:"Assustador � true crime",provider:"elevenlabs"},

    {id:"suno",label:"?? Suno AI",desc:"M�sica completa com melodia",provider:"suno"},

    {id:"udio",label:"?? Udio",desc:"M�sica + voz cantada",provider:"udio"},
  ...
  return(
    ...
    <div className="page">
      ...
      <div className="page-header"><div className="eyebrow"><div className="eyebrow-dot"/>IA & Cria��o</div><h1 className="page-title">Gerador de <em>M�sica</em></h1><p className="page-sub">Trilhas com ElevenLabs, Suno AI e Udio.</p></div>
      ...
      <div className="card" style={{marginBottom:"1rem"}}>
        ...
        <div className="field"><label className="label">Dura��o</label><div className="chips">{[5,10,15,22].map(d=><button key={d} className={"chip "+(duracao===d?"active":"")} onClick={()=>setDuracao(d)} disabled={loading}>{d}s</button>)}</div></div>
        ...
        <button className="btn btn-full" onClick={gerar} disabled={loading}>{loading?"? Compondo...":"?? Gerar M�sica � 3 cr�ditos"}</button>
      ...
      <div className="card">
        ...
        <div className="card-title" style={{marginBottom:12}}>? M�sica gerada!</div>
...
function ScoreViral() {
  ...
  const extrairScores=(txt)=>{
    ...
    const dims=[
      ...
      {key:"retencao",label:"Reten��o",icon:"???",color:"#22d3ee"},

      {key:"emocao",label:"Emo��o",icon:"??",color:"#f87171"},

      {key:"shares",label:"Shares",icon:"??",color:"#34d399"},

      {key:"comentario",label:"Coment�rios",icon:"??",color:"#fbbf24"},
    ...
    const mediaMatch=txt.match(/m�dia[^0-9]*(\d+(?:\.\d+)?)/i)||txt.match(/score[^0-9]*(\d+(?:\.\d+)?)/i)||txt.match(/M�DIA[^0-9]*(\d+(?:\.\d+)?)/i);
  ...
  return(
    ...
    <div className="page">
      ...
      <div className="page-header">
        ...
        <p className="page-sub">Analise o potencial do seu roteiro em 5 dimens�es.</p>
        ...
        <div style={{display:"inline-flex",alignItems:"center",gap:6,marginTop:6,
          ...
          <span style={{fontSize:11,color:"#34d399",fontWeight:600}}>Gr�tis e ilimitado � use sem cr�ditos</span>
      ...
      <div className="card" style={{marginBottom:"1rem"}}>
        ...
        <button className="btn btn-full" onClick={analisar} disabled={loading||!roteiro.trim()}>
          ...
          {loading?<><div className="spinner"/>Analisando com IA...</>:"? Analisar Score Viral � Gr�tis 8"}
        ...
        <div>
          ...
          {sc.dims.length>0&&(
            ...
            <div className="card" style={{marginBottom:12}}>
              ...
              <div className="card-title" style={{marginBottom:12}}>?? 5 Dimens�es</div>
          ...
          <div className="card" style={{marginBottom:12}}>
            ...
            <div className="card-title" style={{marginBottom:10}}>?? An�lise completa</div>
          ...
          {!feedbackDado&&(
            ...
            <div className="card" style={{background:"rgba(123,47,255,.04)",border:".5px solid rgba(123,47,255,.15)"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                ...
                <span style={{fontSize:12,color:"var(--text3)",flex:1}}>?? Esse score foi �til?</span>
              ...
              <div style={{display:"flex",gap:8}}>
                ...
                }} className="btn" style={{flex:1,fontSize:13}}>?? �til</button>
...
function MemoriaVortex() {
  ...
  const CAMPOS=[
    ...
    {key:"publico_alvo",label:"P�blico-alvo",icon:"??",placeholder:"Ex: jovens 18-25, gamers, m�es empreendedoras"},

    {key:"formatos_favoritos",label:"Formatos favoritos",icon:"??",placeholder:"Ex: talking head, POV, trending sounds"},

    {key:"evitar",label:"O que evitar",icon:"??",placeholder:"Ex: palavr�o, pol�tica, assuntos pesados"},

    {key:"cta_favorito",label:"CTA favorito",icon:"??",placeholder:"Ex: Me segue pra mais! Comenta X se concordar"},

    {key:"referencias",label:"Refer�ncias/inspira��es",icon:"?",placeholder:"Ex: @fulano, Nubank, estilo Hormozi"},
  ...
  return(
    ...
    <div className="page">
      ...
      <div className="page-header">
        ...
        <div className="eyebrow"><div className="eyebrow-dot"/>Personaliza��o</div>

        <h1 className="page-title">Mem�ria do <em>Vortex</em></h1>

        <p className="page-sub">O Vortex aprende com voc�. Essas informa��es ficam em todos os roteiros.</p>
      ...
      <div className="card" style={{marginBottom:12,background:"rgba(123,47,255,.04)",border:".5px solid rgba(123,47,255,.15)"}}>
        ...
        <div style={{display:"flex",gap:12,alignItems:"center"}}>
          ...
          <div style={{width:44,height:44,borderRadius:12,background:"linear-gradient(135deg,#7b2fff,#9d5cff)",
            ...
            {perfil.nicho==="terror"?"??":perfil.nicho==="gaming"?"??":perfil.nicho==="finan�as"?"??":"??"}
          ...
          <div>
            ...
            <div style={{fontSize:12,color:"var(--text3)"}}>
              ...
              {perfil.nicho||"nicho n�o definido"} � {perfil.plataforma||"plataforma n�o definida"}
          ...
          <div style={{marginLeft:"auto",fontSize:11,color:"#7b2fff"}}>
            ...
            {Object.keys(mem).length} mem�rias salvas
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <div>
                ...
                {!mem[c.key]&&editando!==c.key&&(
                  ...
                  <div style={{fontSize:11,color:"var(--text3)"}}>N�o definido</div>
      ...
      <div style={{textAlign:"center",fontSize:12,color:"var(--text3)",padding:"8px 0"}}>
        ...
        ?? Essas mem�rias s�o usadas automaticamente em roteiros, chat e an�lises.
...
function MeusProjetos() {
  ...
  return(<div className="page"><div className="page-header"><div className="eyebrow"><div className="eyebrow-dot"/>meu espa�o</div><h1 className="page-title">Meus <em>Projetos</em></h1><p className="page-sub">Hist�rico de cria��es.</p></div>{historico.length===0&&<div className="card" style={{textAlign:"center",padding:"3rem"}}><div style={{fontSize:48,marginBottom:12}}>??</div><p style={{color:"var(--text3)"}}>Nenhum projeto ainda.</p></div>}{historico.filter(h=>h.tipo==="debito").reverse().map((h,i)=><div key={i} className="card" style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",marginBottom:6}}><span style={{fontSize:22,flexShrink:0}}>{ICONS[h.operacao?.split("_").slice(0,2).join("_")]||"??"}</span><div style={{flex:1}}><div style={{fontSize:13,fontWeight:500,color:"var(--text)",textTransform:"capitalize"}}>{(h.operacao||"").replace(/_/g," ")}</div><div style={{fontSize:11,color:"var(--text3)"}}>{new Date(h.timestamp).toLocaleString("pt-BR",{day:"2-digit",month:"2-digit",hour:"2-digit",minute:"2-digit"})}</div></div><div style={{fontSize:12,color:"#f87171",fontWeight:600,flexShrink:0}}>-{h.quantidade} ??</div></div>)}</div>);
...
function useCurrency() {
  ...
  // Moedas internacionais desativadas � s� BRL por enquanto
  ...
  return (
    ...
    <div className="page">
      ...
      <div className="page-header">
        ...
        <p className="page-sub">Todos os planos com cr�ditos que renovam todo m�s.</p>
      ...
      {(preLaunch||isPromo)&&(
        ...
        <div className="card" style={{marginBottom:12,background:"rgba(123,47,255,.06)",border:".5px solid rgba(123,47,255,.2)",textAlign:"center"}}>
          ...
          <div style={{fontSize:11,color:"var(--text3)",marginBottom:6,letterSpacing:".1em"}}>
            ...
            {preLaunch?"?? LAN�AMENTO EM":"? PRE�O DE LAN�AMENTO POR"}
          ...
          {showPromo&&<div style={{fontSize:11,color:"#34d399",marginTop:6}}>?? Pre�o de lan�amento � garanta agora e mant�m para sempre!</div>}
        ...
        <div style={{display:"flex",gap:6}}>
          ...
          {["mensal","creditos"].map(t=>(
            ...
            <button key={t} onClick={()=>setTab(t)} style={{padding:"7px 18px",borderRadius:99,border:`.5px solid ${tab===t?"rgba(123,47,255,.5)":"var(--border2)"}`,background:tab===t?"rgba(123,47,255,.15)":"transparent",color:tab===t?"var(--purple2)":"var(--text3)",fontFamily:"var(--fh)",fontSize:12,fontWeight:700,cursor:"pointer"}}>
              ...
              {t==="mensal"?"?? Mensal":"?? Cr�ditos Avulsos"}
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              ...
              <div>
                ...
                <div style={{fontSize:14,fontWeight:700,color:"var(--text)"}}>Plano Free � R$0/m�s</div>

                <div style={{fontSize:11,color:"var(--text3)"}}>50 cr�ditos � ?? Tend�ncias ilimitado � ? Score ilimitado � 3 roteiros/dia</div>
          ...
          {planos.map(p=>{
            ...
            <div key={p.id} onClick={()=>setSel(p.id)} style={{background:sel===p.id?p.cor+"18":"var(--bg3)",border:`1.5px solid ${sel===p.id?p.cor:"var(--border)"}`,borderRadius:"var(--r-lg)",padding:"1.25rem 1rem",cursor:"pointer",transition:"all .2s",position:"relative"}}>
              ...
              <div style={{display:"flex",alignItems:"baseline",gap:6,marginBottom:4}}>
                ...
                <span style={{fontSize:11,color:"var(--text3)"}}>/m�s</span>

              </div>

              {showPromo&&<div style={{fontSize:10,color:"#34d399",marginBottom:4}}>?? {Math.round((1-preco(p)/p.normal)*100)}% off lan�amento</div>}

              <div style={{fontSize:12,color:"var(--text2)",marginBottom:8,fontWeight:600}}>?? {creditos_tab(p).toLocaleString()} cr�ditos/m�s</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:10,marginBottom:"1.5rem"}}>
          ...
          {PACKS.map(p=>(
            ...
            <div key={p.id} onClick={()=>setSel("pack_"+p.id)} style={{background:sel==="pack_"+p.id?"rgba(123,47,255,.08)":"var(--bg3)",border:sel==="pack_"+p.id?"1px solid rgba(123,47,255,.5)":".5px solid var(--border)",borderRadius:"var(--r-lg)",padding:"1.25rem 1rem",cursor:"pointer",transition:"all .2s",position:"relative"}}>
              ...
              <div style={{fontSize:14,fontWeight:700,color:p.cor||"var(--accent)"}}>?? {p.creditos.toLocaleString()} cr�ditos</div>

              <div style={{fontSize:10,color:"var(--text3)",marginTop:2}}>N�o expiram � Uso livre</div>
            ...
            return(
              ...
              <>
                ...
                <div style={{display:"flex",justifyContent:"space-between",fontSize:13,padding:"7px 0",borderBottom:".5px solid var(--border)"}}>
                  ...
                  <span style={{fontWeight:700}}>{fmt(valor)}{!isPack?"/m�s":""}</span>
...
function Configuracoes() {
  ...
  const IDIOMAS=["Portugu�s","Ingl�s","Espanhol"];
  ...
  return(
    ...
    <div className="page">
      ...
      <div className="page-header">
        ...
        <h1 className="page-title">Configu<em>ra��es</em></h1>
      ...
      <div style={{display:"flex",gap:6,marginBottom:16,background:"rgba(255,255,255,.04)",borderRadius:12,padding:4}}>
        ...
        {[{id:"perfil",icon:"??",label:"Perfil"},{id:"preferencias",icon:"??",label:"Prefer�ncias"},{id:"funcoes",icon:"??",label:"Fun��es"}].map(t=>(
      ...
      {aba==="perfil"&&(
        ...
        <div className="card" style={{marginBottom:12}}>
          ...
          <div className="field">
            ...
            <input className="input" placeholder="Ex: Jo�o, @joaocriador" value={perfil.nome||""} onChange={e=>setPer("nome",e.target.value)}/>
          ...
          <div className="field">
            ...
            <label className="label">Idioma dos conte�dos</label>
          ...
          <div className="field">
            ...
            <label className="label">Tom de voz do conte�do</label>

            <div className="chips">

              {["Informal","Profissional","Humor�stico","Dram�tico","Educativo"].map(t=>(
      ...
      {aba==="preferencias"&&(
        ...
        <div className="card" style={{marginBottom:12}}>
          ...
          <div className="card-title" style={{marginBottom:14}}>?? Prefer�ncias do app</div>



          <div className="field">

            <label className="label">Dura��o padr�o do roteiro</label>
          ...
          <div className="field">
            ...
            <label className="label">Modelo padr�o de chat</label>

            <div className="chips">

              {["Auto (melhor dispon�vel)","Claude","GPT-4o","Gemini","Groq (r�pido)"].map(m=>(
          ...
          <div className="field">
            ...
            <label className="label">Pa�s para tend�ncias</label>

            <div className="chips">

              {["???? Brasil","???? EUA","???? Portugal","???? M�xico"].map(p=>(
      ...
      {aba==="funcoes"&&(
        ...
        <div className="card" style={{marginBottom:12}}>
          ...
          <div className="card-title" style={{marginBottom:14}}>?? Fun��es ativas</div>

          {[

            {key:"analise_perfil",name:"An�lise de Perfil",desc:"Analisa perfis de redes sociais",icon:"??"},

            {key:"roteiro",name:"Gerador de Roteiro",desc:"Roteiros virais com IA",icon:"??"},

            {key:"tendencias",name:"Tend�ncias",desc:"Google Trends em tempo real",icon:"??"},

            {key:"gerador_imagem",name:"Imagens IA",desc:"FLUX, DALL-E e mais",icon:"??"},

            {key:"gerador_video",name:"V�deos IA",desc:"Dispon�vel em julho",icon:"??"},

            {key:"clonador_voz",name:"Clonar Voz",desc:"ElevenLabs voice cloning",icon:"???"},

            {key:"musica",name:"Gerar M�sica",desc:"Trilhas para v�deos",icon:"??"},
      ...
      <button className="btn btn-full" onClick={salvar}>?? Salvar configura��es</button>
...
function WelcomeScreen({ onEnter }) {
  ...
  useEffect(() => {
    ...
    // Canvas s� com part�culas sobre a imagem � sem buraco negro desenhado
    ...
    const draw=()=>{
      ...
      // Part�culas saindo do centro (buraco negro na imagem)
      ...
      stars.forEach(s=>{
        ...
        // Move part�culas para fora do centro (efeito v�rtice)
  ...
  useEffect(()=>{
    ...
    const LINES=[
      ...
      {h:"Crie conte�do\nque viraliza.",s:"Roteiros, hooks e tend�ncias com IA."},
      ...
      {h:"Trends antes\nde todos.",s:"Fique um passo � frente do algoritmo."},



      {h:"Seu criativo.\nNossa intelig�ncia.",s:"Est�dio de IA para criadores brasileiros."},
...
function Onboarding({ onConcluir }) {
  ...
  async function concluir(){
    ...
    } catch { setBoasVindas("Pronto! Vamos criar conte�do viral! ??"); }
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    const verificarAuth=async()=>{
      ...
      try{
        ...
        if(params.get("pagamento")==="ok"){window.history.replaceState({},"",window.location.pathname);setTimeout(()=>alert("? Pagamento confirmado! Cr�ditos adicionados."),1000);}
        ...
        if(r&&r.ok){
          ...
          setScreen("app"); // j� logado ? pular welcome
  ...
  // Modal de login � aparece quando usu�rio tenta usar feature paga
  ...
  // Fun��o global para pedir login

  window.__vortexPedirLogin = (msg="Fa�a login para continuar") => {
          ...
          <main className="main">
            ...
            <header className="topbar">
              ...
              <div className="topbar-right">
                ...
                <span className="pill pill-purple" style={{cursor:"pointer"}} onClick={()=>setAba("creditos")}>? {saldo} cr�ditos</span>
